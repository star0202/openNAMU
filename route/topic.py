...
def topic(topic_num=0, do_type="", doc_name="Test"):
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST" and do_type == "":
            ...
            if topic_num == "0":
                curs.execute(
                    db_change("select code from topic order by code + 0 desc limit 1")
                )
            ...
            if match:
                ...
                if ip_or_user(match) == 1:
                    curs.execute(
                        db_change("select ip from history where ip = ? limit 1"),
                        [match],
                    )
                    ...
                    if u_data:
                        ...
                        curs.execute(
                            db_change("select ip from topic where ip = ? limit 1"),
                            [match],
                        )
                        u_data = curs.fetchall()
                        if u_data:
                    ...
                    curs.execute(
                        db_change("select id from user_set where id = ?"), [match]
                    )
                    u_data = curs.fetchall()
                    if u_data:
                        y_check = 1
            ...
            data = cate_re.sub(
                "[br]", flask.request.form.get("content", "Test").replace("\r", "")
            )
            ...
            while 1:
                ...
                if call_thread_count < 0:
                    ...
                    curs.execute(
                        db_change("select ip from topic where code = ? and id = ?"),
                        [topic_num, rd_data[1]],
                    )
                    ip_data = curs.fetchall()
                    if (
                        ip_data
                        and ip_or_user(ip_data[0][0]) == 0
                        and ip != ip_data[0][0]
                    ):
                        add_alarm(
                            ip_data[0][0],
            ...
            do_add_thread(topic_num, data, "", num)
            do_reload_recent_thread(topic_num, today, name, sub)
            ...
            if topic_num == "0":
                ...
                curs.execute(
                    db_change("select title, sub from rd where code = ?"), [topic_num]
                )
                name = curs.fetchall()
                if name:
                    sub = name[0][1]
            ...
            if do_type == "preview":
                ...
                thread_data_preview = render_set(
                    doc_name="", doc_data=thread_data, data_in=""
                )
            ...
            topic_text = (
                ...
                else load_lang("content")
            )

            shortcut = '<div class="opennamu_thread_shortcut" id="thread_shortcut">'
            curs.execute(
                db_change("select id from topic where code = ? order by id + 0 asc"),
                [topic_num],
            )
            ...
            json_data = json.loads(
                api_topic(int(topic_num), "normal", "", "render").data
            )
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("discussion") + ")", 0]),
                    ],
                    data="""
                    """
                    + shortcut
                    + """
                    <h2 id="topic_top_title">"""
                    + html.escape(sub)
                    + """</h2>
                    ...
                    + '''</div>
                        ...
                        <textarea id="opennamu_edit_textarea" class="opennamu_textarea_100" placeholder="'''
                    + topic_text
                    + """" name="content">"""
                    + html.escape(thread_data)
                    + """</textarea>
                        <hr class="main_hr">
                    ...
                    + """
                        ...
                        <button id="opennamu_save_button" formaction="/thread/"""
                    + topic_num
                    + """" type="submit">"""
                    + load_lang("send")
                    + """</button>
                        <button id="opennamu_preview_button" formaction="/thread_preview/"""
                    + topic_num
