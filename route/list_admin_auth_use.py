...
def list_admin_auth_use(arg_num=1, arg_search="normal"):
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            return redirect(
                "/list/admin/auth_use/"
                + url_pas(flask.request.form.get("search", "normal"))
                + "/1"
            )
        else:
            if arg_search == "normal":
                curs.execute(
                    db_change(
                        "select who, what, time from re_admin order by time desc limit ?, 50"
                    ),
                    [sql_num],
                )
            else:
            ...
            list_data += "</ul>"
            list_data += next_fix(
                "/list/admin/auth_use/" + url_pas(arg_search) + "/", arg_num, get_list
            )
