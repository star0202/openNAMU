...
def recent_block_2(name, tool):
    with get_db_connect() as conn:
        ...
        if tool == "all":
            if get_type == "ongoing":
                ...
                if sub_type == "":
                    div = (
                        ""
                        + '<a href="?type=ongoing&s_type=regex">('
                        + load_lang("regex")
                        + ")</a> "
                        + '<a href="?type=ongoing&s_type=normal">('
                        + load_lang("normal")
                        + ")</a>"
                        + '<hr class="main_hr">'
                        + ""
        ...
        if tool == "all":
            ...
            div += next_fix(
                "/block_log/" + url_pas(tool) + "/" + url_pas(name) + "?num=",
                num,
                data_list,
            )
