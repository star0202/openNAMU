...
def vote_add():
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            curs.execute(
                db_change(
                    'select id from vote where not type = "option" order by id + 0 desc limit 1'
                )
            )
            ...
            curs.execute(
                db_change(
                    "insert into vote (name, id, subject, data, user, type, acl) values (?, ?, ?, ?, '', ?, ?)"
                ),
                [
            ...
            if time_limitless == "":
                ...
                if re.search(r"^[0-9]{4}-[0-9]{2}-[0-9]{2}$", time_limit):
                    curs.execute(
                        db_change(
                            "insert into vote (name, id, subject, data, user, type, acl) values ('end_date', ?, '', ?, '', 'option', '')"
                        ),
                        [id_data, time_limit],
                    )
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    + '">'
                    + '<hr class="main_hr">'
                    + '<textarea class="opennamu_textarea_100" name="subject" placeholder="'
                    + load_lang("explanation")
                    + '"></textarea>'
                    + '<hr class="main_hr">'
                    + '<textarea class="opennamu_textarea_500" name="data" placeholder="'
                    + load_lang("1_line_1_q")
                    + '"></textarea>'
                    + '<hr class="main_hr">'
                    + '<input type="checkbox" value="Y" name="open_select"> '
                    + load_lang("open_vote")
                    + "<h2>"
                    + load_lang("period")
                    + "</h2>"
                    ...
                    + '<hr class="main_hr">'
                    + '<input type="checkbox" value="Y" name="limitless"> '
                    + load_lang("limitless")
                    + "<h2>"
                    + load_lang("acl")
                    + "</h2>"
