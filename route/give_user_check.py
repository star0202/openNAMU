...
def give_user_check_2(name):
    with get_db_connect() as conn:
        ...
        if admin_check("all", None, name) == 1 or (
            plus_id and admin_check("all", None, plus_id) == 1
        ):
            if admin_check() != 1:
        ...
        if (
            admin_check(
                4,
                (check_type + " " if check_type != "" else "") + "check (" + name + ")",
            )
            != 1
        ):
        ...
        if check_type == "":
            if ip_or_user(name) == 0:
                curs.execute(
                    db_change(
                        'select data from user_set where name = "approval_question" and id = ?'
                    ),
                    [name],
                )
                ...
                if approval_question and approval_question[0][0]:
                    curs.execute(
                        db_change(
                            'select data from user_set where name = "approval_question_answer" and id = ?'
                        ),
                        [name],
                    )
                            ...
                            <table id="main_table_set">
                                <tbody>
                                    <tr id="main_table_top_tr">
                                        <td>Q</td>
                                        <td>"""
                            + approval_question[0][0]
                            + """</td>
                                        <td>A</td>
                                        <td>"""
                            + approval_question_answer[0][0]
                            + """</td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                if num == 1:
                    curs.execute(
                        db_change(
                            ...
                            + "where "
                            + ("ip" if ip_or_user(name) == 1 else "name")
                            + " = ? or "
                            + ("ip" if ip_or_user(plus_id) == 1 else "name")
                            + " = ?"
                            ""
                        ),
                        [name, plus_id],
                ...
                for data in record:
                    if data[2]:
                        if len(data[2]) > 300:
                            ua = (
                                ""
                                + '<a href="javascript:void();" onclick="document.getElementById(\'check_'
                                + str(set_n)
                                + "').style.display='block';\">(300+)</a>"
                                + '<div id="check_'
                                + str(set_n)
                                + '" style="display:none;">'
                                + html.escape(data[2])
            ...
            div += next_fix(
                "/check/"
                + url_pas(name)
                + ("?plus=" + plus_id + "&num=" if plus_id else "?num="),
                num,
                record,
            )
            ...
            curs.execute(
                db_change(
                    ""
                    + "select distinct "
                    + ("name" if ip_or_user(name) == 1 else "ip")
                    + ", today from ua_d "
                    + "where "
                    + ("ip" if ip_or_user(name) == 1 else "name")
                    + " = ? "
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("simple_check") + ")", 0]),
                    ],
                    data=div,
