...
def view_read(name="Test", doc_rev="", doc_from="", do_type=""):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                "select sub from rd where title = ? and not stop = 'O' order by date desc"
            ),
            [name],
        )
        ...
        curs.execute(
            db_change("select title from data where title like ?"), [name + "/%"]
        )
        ...
        if re.search(r"^category:", name):
            ...
            curs.execute(
                db_change(
                    "select link from back where title = ? and type = 'cat' order by link asc"
                ),
                [name],
            )
            ...
            for data in category_sql:
                if data[0].startswith("category:"):
                    ...
                    category_doc += (
                        ...
                        + html.escape(data[0])
                        + "</a> "
                        + '<a class="opennamu_link_inter" href="/xref/'
                        + url_pas(data[0])
                        + '">('
                        + load_lang("backlink")
                        + ")</a>"
            ...
            if admin_check("all", None, user_name) == 1:
                if admin_check(None, None, user_name) == 1:
                    curs.execute(
                        db_change(
                            'select data from other where name = "phrase_user_page_owner"'
                        )
                    )
                    ...
                    if db_data and db_data[0][0] != "":
                        ...
                        curs.execute(
                            db_change(
                                'select data from other where name = "phrase_user_page_admin"'
                            )
                        )
                        db_data = curs.fetchall()
                        if db_data and db_data[0][0] != "":
            ...
            if name == "user:" + user_name:
                menu += [
                    [
                        "w/" + url_pas(name) + "/" + url_pas(get_time().split()[0]),
                        load_lang("today_doc"),
                    ]
                ]
            ...
            if os.path.exists(file_path_name):
                ...
                file_data = (
                    """
                    <img src="/image/"""
                    + url_pas(file_all_name)
                    + """">
                    <h2>DATA</h2>
                    <table>
                        <tr><td>URL</td><td><a href="/image/"""
                    + url_pas(file_all_name)
                    + """">LINK</a></td></tr>
                        <tr><td>VOLUME</td><td>"""
                    + file_size
        ...
        if num != "":
            curs.execute(
                db_change(
                    "select title from history where title = ? and id = ? and hide = 'O'"
                ),
                [name, num],
            )
            ...
            curs.execute(
                db_change("select data from history where title = ? and id = ?"),
                [name, num],
            )
        else:
        ...
        if end_data == "HTTP Request 401.3":
            ...
            if sql_d:
                end_data += (
                    "<h2>" + load_lang("history") + '</h2><ul class="opennamu_ul">'
                )
                for i in sql_d:
                    if re.search(r"\+", i[2]):
                ...
                end_data += (
                    '<li><a href="/history/' + url_pas(name) + '">(...)</a></li></ul>'
                )
        else:
            response_data = 200
            ...
            if do_type == "from":
                ...
                if flask.session and "lastest_document" in flask.session:
                    end_data = (
                        ...
                        <div id="redirect">
                            <a href="/w_from/"""
                        + url_pas(flask.session["lastest_document"])
                        + """">"""
        ...
        if num != "":
            curs.execute(
                db_change(
                    'select data from other where name = "phrase_old_page_warring"'
                )
            )
            db_data = curs.fetchall()
            if db_data and db_data[0][0] != "":
        ...
        if ip_or_user(ip) == 0:
            curs.execute(
                db_change("select title from scan where user = ? and title = ?"),
                [ip, name],
            )
            ...
            easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css([sub, r_date, watch_list]),
                    ],
                    data=div,
                    menu=menu,
                )
