...
def topic_tool_setting(topic_num=1):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change("select stop, agree from rd where code = ?"), [topic_num]
        )
        rd_d = curs.fetchall()
        if not rd_d:
            return redirect("/")
        ...
        if flask.request.method == "POST":
            ...
            if stop_d != rd_d[0][0]:
                curs.execute(
                    db_change("update rd set stop = ? where code = ?"),
                    [stop_d, topic_num],
                )
                ...
                do_add_thread(topic_num, load_lang(t_state), "1")

            if agree_d != rd_d[0][1]:
                curs.execute(
                    db_change("update rd set agree = ? where code = ?"),
                    [agree_d, topic_num],
                )
            ...
            if why_d != "":
                do_add_thread(topic_num, load_lang("why") + " : " + why_d, "1")
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    <form method="post">
                        ...
                        <hr class="main_hr">
                        <input type="checkbox" name="agree" value="O" """
                        + agree_check
                        + """> """
                        + load_lang("topic_change_agree")
                        + """

                        <h2>"""
                        + load_lang("topic_associate")
                        + """</h2>
                        """
                        + load_lang("topic_link_vote")
                        + """ ("""
                        + load_lang("not_working")
                        + ''')
                        <hr class="main_hr">
                        <input placeholder="'''
                        + load_lang("topic_insert_vote_number")
                        + """" name="vote_number" type="number">
