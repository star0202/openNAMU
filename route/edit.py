...
def edit(name="Test", section=0, do_type=""):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change("select id from history where title = ? order by id + 0 desc"),
            [name],
        )
        ...
        if edit_repeat == "post":
                ...
                if section != "":
                    if flask.request.form.get("doc_section_edit_apply", "X") != "X":
                        if flask.request.form.get("doc_section_data_where", "") != "":
                            data_match_where = flask.request.form.get(
                            ...
                            if len(data_match_where) == 2:
                                ...
                                if data_match_where[1] != "inf":
                                    data_match_b = int(
                                        number_check(data_match_where[1])
                                    )
                                else:
                                    data_match_b = "inf"
            ...
            render_set(doc_name=name, doc_data=content, data_in="")

            if db_data:
                curs.execute(
                    db_change("update data set data = ? where title = ?"),
                    [content, name],
                )
            else:
                curs.execute(
                    db_change("insert into data (title, data) values (?, ?)"),
                    [name, content],
                )
                ...
                curs.execute(
                    db_change(
                        "update other set data = ? where name = 'count_all_title'"
                    ),
                    [str(int(curs.fetchall()[0][0]) + 1)],
                )

            curs.execute(
                db_change("select user from scan where title = ? and type = ''"), [name]
            )
            ...
            history_plus(name, content, today, ip, send, leng)

            curs.execute(db_change("delete from back where link = ?"), [name])
            curs.execute(
                db_change("delete from back where title = ? and type = 'no'"), [name]
            )

            render_set(doc_name=name, doc_data=content, data_type="backlink")
            ...
            if edit_repeat == "get":
                ...
                curs.execute(
                    db_change("select data from data where title = ?"), [load_title]
                )
                db_data = curs.fetchall()
                ...
                if section != "":
                    curs.execute(
                        db_change('select data from other where name = "markup"')
                    )
                    db_data = curs.fetchall()
                    ...
                    if db_data in ("namumark", "namumark_beta"):
                        ...
                        while 1:
                            ...
                            if section == count:
                                ...
                                data_match_plus = re.search(
                                    data_match_re, data_section_sub
                                )
                                if data_match_plus:
                                    data_section = data[
                                        data_match.span()[0] : data_match_plus.span()[0]
                                        - 1
                                    ]
                                    data_section_where = (
                                        str(data_match.span()[0])
                                        + ","
                                        + str(data_match_plus.span()[0] - 1)
                                    )
                                else:
                                    data_section = data[data_match.span()[0] :]
                                    data_section_where = (
                                        str(data_match.span()[0]) + ",inf"
                                    )
                ...
                data_section_where = flask.request.form.get(
                    "doc_section_data_where", ""
                )
                doc_section_edit_apply = flask.request.form.get(
                    "doc_section_edit_apply", ""
                )
                ...
                if do_type != "preview":
                    ...
                    data_preview = render_set(
                        doc_name=name, doc_data=data, data_in="from"
                    )
            ...
            if section == "":
                ...
                form_action_preview = (
                    'formaction="/edit_section_preview/'
                    + str(section)
                    + "/"
                    + url_pas(name)
                    + '"'
            ...
            sql_d = curs.fetchall()
            p_text = (
                html.escape(sql_d[0][0])
                if sql_d and sql_d[0][0] != ""
                else load_lang("default_edit_help")
            )
            ...
            if monaco_on == "use":
                ...
                editor_top_text += (
                    ' <a href="javascript:opennamu_edit_turn_off_monaco();">('
                    + load_lang("turn_off_monaco")
                    + ")</a>"
                )
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("edit") + ")" + sub_menu, 0]),
                    ],
                    ...
                    <form method="post">
                        <textarea style="display: none;" id="opennamu_edit_origin" name="doc_data_org">"""
                    + html.escape(data_section)
                    + """</textarea>
                        <textarea style="display: none;" name="doc_section_data_where">"""
                    + data_section_where
                    + '''</textarea>
                        <input style="display: none;" name="doc_section_edit_apply" value="'''
                    + doc_section_edit_apply
                    + '''">

                        <input style="display: none;" name="ver" value="'''
                    + doc_ver
                    + """">
                        
                        <div>"""
                    + edit_button("opennamu_edit_textarea", "opennamu_monaco_editor")
                    + """</div>
                        
                        <div id="opennamu_monaco_editor" class="opennamu_textarea_500" """
                    + monaco_display
                    + """></div>
                        <textarea id="opennamu_edit_textarea" """
                    + editor_display
                    + ''' class="opennamu_textarea_500" name="content" placeholder="'''
                    + p_text
                    + """">"""
                    + html.escape(data_section)
                    + '''</textarea>
                    ...
                    + """
                        ...
                        <button id="opennamu_save_button" type="submit" """
                    + form_action
                    + """ onclick="do_monaco_to_textarea(); do_stop_exit_release();">"""
                    + load_lang("save")
                    + """</button>
                        <button id="opennamu_preview_button" type="submit" """
                    + form_action_preview
                    + """ onclick="do_monaco_to_textarea(); do_stop_exit_release();">"""
                    + load_lang("preview")
                    + """</button>
