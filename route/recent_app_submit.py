...
def recent_app_submit_2():
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change('select data from other where name = "requires_approval"')
        )
        ...
        if flask.request.method == "GET":
            curs.execute(
                db_change('select data from user_set where name = "application"')
            )
            db_data = curs.fetchall()
            if db_data:
                div += (
                    ""
                    + load_lang("all_register_num")
                    + " : "
                    + str(len(db_data))
                    + '<hr class="main_hr">'
                    + ""
            ...
            if flask.request.form.get("approve", "") != "":
                ...
                curs.execute(
                    ...
                    [application["id"], application["question"]],
                )
                curs.execute(
                    db_change(
                        "insert into user_set (name, id, data) values ('approval_question_answer', ?, ?)"
                    ),
                    [application["id"], application["answer"]],
                )
