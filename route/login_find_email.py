    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            i_text = (
                ...
                if sql_d and sql_d[0][0] != ""
                else ("Key : " + flask.session["c_key"])
            )

            if tool == "pass_find":
                curs.execute(
                    db_change(
                        "select id from user_set where id = ? and name = 'email' and data = ?"
                    ),
                    [user_id, user_email],
                )
                if not curs.fetchall():
                ...
                curs.execute(
                    db_change(
                        'select id from user_set where name = "email" and data = ?'
                    ),
                    [user_email],
                )
                if curs.fetchall():
            ...
            if tool == "pass_find":
                curs.execute(
                    db_change(
                        'select data from other where name = "password_search_text"'
                    )
                )
                sql_d = curs.fetchall()
                b_text = (
                ...
                curs.execute(
                    db_change('select data from other where name = "email_insert_text"')
                )
                sql_d = curs.fetchall()
                b_text = (
                    (sql_d[0][0] + '<hr class="main_hr">')
