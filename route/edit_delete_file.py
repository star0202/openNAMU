...
def edit_delete_file(name: str = "test.jpg") -> str:
    with get_db_connect() as conn:
        curs: typing.Union[
            sqlite3.dbapi2.Cursor, pymysql.cursors.Cursor, None
        ] = conn.cursor()
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("file_delete") + ")", 0]),
                    ],
                    data="""
                    <form method="post">
                        <img src="/image/"""
                    ...
                    + """</a>
                        <hr class="main_hr">
                        <input name="with_doc" type="checkbox" checked> """
                    + load_lang("file_delete_with_document")
                    + """
                        <hr class="main_hr">
                        <button type="submit">"""
                    + load_lang("file_delete")
                    + """</button>
                    </form>
                """,
