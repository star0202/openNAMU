    ...
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                ...
                + "where "
                + sql_insert[1]
                + " = ? and not type = 'no' and not type = 'nothing'"
                + "order by type asc, "
                + sql_insert[0]
                + " asc limit ?, 50"
                + ""
        ...
        for data in data_list:
            ...
            curs.execute(
                db_change(
                    "select title from back where title = ? and type = 'include'"
                ),
                [data[0]],
            )
        ...
        div += "</ul>" + next_fix(
            "/xref/" + url_pas(name) + "?change=" + str(xref_type) + "&num=",
            num,
            data_list,
        )
        ...
        return easy_minify(
            flask.render_template(
                ...
                menu=[
                    ...
                    ["xref_reset/" + url_pas(name), load_lang("reset_backlink")],
                ],
            )
        )
