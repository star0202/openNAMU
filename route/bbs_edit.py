...
def bbs_edit(bbs_num="", post_num="", do_type=""):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                'select set_id from bbs_set where set_id = ? and set_name = "bbs_name"'
            ),
            [bbs_num_str],
        )
        ...
        if flask.request.method == "POST" and do_type != "preview":
            ...
            if post_num == "":
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('title', ?, ?, ?)"
                    ),
                    [id_data, bbs_num_str, title],
                )
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('data', ?, ?, ?)"
                    ),
                    [id_data, bbs_num_str, data],
                )
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('date', ?, ?, ?)"
                    ),
                    [id_data, bbs_num_str, date],
                )
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('user_id', ?, ?, ?)"
                    ),
                    [id_data, bbs_num_str, ip],
                )
            else:
                curs.execute(
                    db_change(
            ...
            if do_type == "preview":
                ...
                data_preview = render_set(doc_name="", doc_data=data, data_in="from")
            else:
                if post_num == "":
                    title = ""
                    data = ""
            ...
            if post_num == "":
                ...
                form_action_preview = (
                    'formaction="/bbs/edit/preview/' + bbs_num_str + '"'
                )
            else:
                form_action = (
                    'formaction="/bbs/edit/' + bbs_num_str + "/" + post_num_str + '"'
                )
                form_action_preview = (
                    'formaction="/bbs/edit/preview/'
                    + bbs_num_str
                    + "/"
                    + post_num_str
                    + '"'
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    + editor_display
                    + """ class="opennamu_textarea_500" name="content">"""
                    + html.escape(data)
                    + """</textarea>
                        <hr class="main_hr">
