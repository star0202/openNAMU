...
def filter_inter_wiki(tool):
    with get_db_connect() as conn:
        ...
        if tool == "inter_wiki":
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'inter_wiki'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'email'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'name'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'regex_filter'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'file'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'image_license'"
                )
            )
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'extension'"
                )
            )
        else:
            ...
            curs.execute(
                db_change(
                    "select html, plus, plus_t from html_filter where kind = 'edit_top'"
                )
            )
        ...
        for data in db_data:
            ...
            if tool == "inter_wiki":
                div += (
                    '<td><a class="opennamu_link_out" href="'
                    + html.escape(data[1])
                    + '">'
                    + html.escape(data[1])
                    + "</a></td>"
