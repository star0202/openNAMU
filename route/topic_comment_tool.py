...
def topic_comment_tool(topic_num=1, num=1):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change("select block, ip, date from topic where code = ? and id = ?"),
            [topic_num, num],
        )
        data = curs.fetchall()
        if not data:
        ...
        if admin_check(3) == 1:
            ...
            curs.execute(
                db_change("select end from rb where block = ? and ongoing = '1'"),
                [data[0][1]],
            )
