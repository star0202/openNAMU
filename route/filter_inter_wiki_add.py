        ...
        if flask.request.method == "POST":
            ...
            if tool == "plus_inter_wiki":
                if name:
                    curs.execute(
                        db_change(
                            "delete from html_filter where html = ? and kind = 'inter_wiki'"
                        ),
                        [name],
                    )
                ...
                curs.execute(
                    db_change(
                        'insert into html_filter (html, plus, plus_t, kind) values (?, ?, ?, "inter_wiki")'
                    ),
                    [
                        flask.request.form.get("title", "test"),
                ...
                try:
                    ...
                    curs.execute(
                        db_change(
                            "delete from html_filter where html = ? and kind = 'regex_filter'"
                        ),
                        [name],
                    )
                    curs.execute(
                        db_change(
                            "insert into html_filter (html, plus, plus_t, kind) values (?, ?, ?, 'regex_filter')"
                        ),
                        [name, flask.request.form.get("content", "test"), end],
                    )
                except:
                ...
                if name:
                    curs.execute(
                        db_change(
                            "delete from html_filter where html = ? and kind = ?"
                        ),
                        [name, type_d],
                    )
                ...
                curs.execute(
                    ...
                    [flask.request.form.get("title", "test"), type_d, plus_d, ""],
                )
            ...
            if tool == "plus_inter_wiki":
                if name:
                    curs.execute(
                        db_change(
                            "select html, plus, plus_t from html_filter where html = ? and kind = 'inter_wiki'"
                        ),
                        [name],
                    )
                    exist = curs.fetchall()
                ...
                curs.execute(
                    db_change(
                        "select plus, plus_t from html_filter where html = ? and kind = 'regex_filter'"
                    ),
                    [name],
                )
                exist = curs.fetchall()
                ...
                if stat == "":
                    ...
                    for i in t_data:
                        insert_data += (
                            "<a href=\"javascript:insert_v('second', '"
                            + i[0]
                            + "')\">("
                            + i[1]
                            + ")</a> "
                ...
                form_data = (
                    """
                    <script>function insert_v(name, data) { document.getElementById(name).value = data; }</script>"""
                    + insert_data
                    + '''
                    <hr class="main_hr">
                    <input placeholder="'''
                    + load_lang("second")
                    + '''" id="second" name="second" type="text" value="'''
                    + html.escape(time_data)
                    + '''">
                    <hr class="main_hr">
                    <input placeholder="'''
                    + load_lang("regex")
                    + '''" name="content" value="'''
                    + html.escape(textarea)
                    + """" type="text">
                """
                )
                ...
                if name:
                    curs.execute(
                        db_change(
                            "select plus from html_filter where html = ? and kind = 'edit_top'"
                        ),
                        [name],
                    )
                    exist = curs.fetchall()
