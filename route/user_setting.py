    ...
    with get_db_connect() as conn:
        ...
        if ip_or_user(ip) == 0:
            if flask.request.method == "POST":
                ...
                for auto_data in auto_list:
                    ...
                    if curs.fetchall():
                        curs.execute(
                            db_change(
                                "update user_set set data = ? where name = ? and id = ?"
                            ),
                            [auto_data[1], auto_data[0], ip],
                        )
                    else:
                        curs.execute(
                            db_change(
                ...
                curs.execute(
                    db_change(
                        'select data from user_set where name = "random_key" and id = ?'
                    ),
                    [ip],
                )
                data = curs.fetchall()
                ...
                curs.execute(
                    db_change(
                        'select data from user_set where name = "lang" and id = ?'
                    ),
                    [ip],
                )
                data = curs.fetchall()
                ...
                for lang_data in support_language:
                    see_data = (
                        lang_data if lang_data != "default" else load_lang("default")
                    )
                ...
                curs.execute(
                    db_change(
                        'select data from user_set where name = "2fa" and id = ?'
                    ),
                    [ip],
                )
                ...
                curs.execute(
                    db_change(
                        'select data from user_set where name = "2fa_pw" and id = ?'
                    ),
                    [ip],
                )
                fa_data_pw = curs.fetchall()
                fa_data_pw = (
                    load_lang("2fa_password_change")
                    if fa_data_pw
                    else load_lang("2fa_password")
                )

                curs.execute(
                    db_change(
                        'select data from user_set where name = "sub_user_name" and id = ?'
                    ),
                    [ip],
                )
                ...
                return easy_minify(
                    flask.render_template(
                        ...
                        data="""
                        <form method="post">
                            <div id="opennamu_get_user_info">"""
                        + ip
                        + """</div>
                            <hr class="main_hr">
                            <a href="/change/pw">("""
                        + load_lang("password_change")
                        + """)</a>
                            <hr class="main_hr">
                            <span>"""
                        + load_lang("email")
                        + """ : """
                        + email
                        + """</span> <a href="/change/email">("""
                        + load_lang("email_change")
                        + """)</a> <a href="/change/email/delete">("""
                        + load_lang("email_delete")
                        + """)</a>
                            <hr class="main_hr">
                            <span>"""
                        + load_lang("password_instead_key")
                        + """ : """
                        + ramdom_key
                        + """ <a href="/change/key">("""
                        + load_lang("key_change")
                        + """)</a> <a href="/change/key/delete">("""
                        + load_lang("key_delete")
                        + """)</a></span>
                            <h2>"""
                        + load_lang("main")
                        + """</h2>
                            <a href="/change/head">("""
                        + load_lang("user_head")
                        + """)</a> <a href="/change/top_menu">("""
                        + load_lang("user_added_menu")
                        + """)</a>
                            <hr class="main_hr">
                            <span>"""
                        + load_lang("skin")
                        ...
                        + load_lang("skin_set")
                        + """)</a> <a href="/change/skin_set/main">("""
                        + load_lang("main_skin_set")
                        + """)</a>
                            <hr class="main_hr">
                            <span>"""
                        + load_lang("language")
                        ...
                        + load_lang("2fa")
                        + """</h2>
                            <select name="2fa" id="twofa_check_input">"""
                        + fa_data_select
                        + '''</select>
                            <hr class="main_hr">
                            <input type="password" name="2fa_pw" placeholder="'''
                        + fa_data_pw
                        + """">
                            <h2>"""
                        + load_lang("sub_user_name")
                        + '''</h2>
                            <input name="sub_user_name" value="'''
                        + html.escape(sub_user_name)
                        + '''" placeholder="'''
                        + load_lang("sub_user_name")
                        + """">
                            <hr class="main_hr">
            ...
            if flask.request.method == "POST":
                ...
                div2 = load_skin(
                    ("" if not "skin" in flask.session else flask.session["skin"]), 0, 1
                )

                data = (
                    [["default"]]
                    if not "lang" in flask.session
                    else [[flask.session["lang"]]]
                )
                div3 = ""
                for lang_data in support_language:
                    see_data = (
