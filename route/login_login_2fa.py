...
def login_login_2fa_2():
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            curs.execute(
                db_change('select data from user_set where name = "2fa_pw" and id = ?'),
                [user_id],
            )
            ...
            if user_1:
                curs.execute(
                    db_change(
                        'select data from user_set where name = "2fa_pw_encode" and id = ?'
                    ),
                    [user_id],
                )
                user_1 = user_1[0][0]
                ...
                pw_check_d = pw_check(
                    flask.request.form.get("pw", ""), user_1, user_2, user_id
                )
                if pw_check_d != 1:
