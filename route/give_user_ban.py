    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            for name in all_user:
                ...
                ban_insert(name, end, why, login, ip_check(), type_d)

            return redirect("/block_log")
        else:
            curs.execute(
                db_change(
                    "select end, why from rb where block = ? and ongoing = '1' and band = ?"
                ),
                [name, ban_type],
            )
            end = curs.fetchall()
            if end:
                ...
                curs.execute(
                    db_change(
                        "select block from rb where block = ? and login = 'O' and ongoing = '1'"
                    ),
                    [name],
                )
                if curs.fetchall():
                ...
                if ban_type == "multiple":
                    ...
                    n_name = (
                        ...
                        + load_lang("name_or_ip_or_regex_multiple")
                        + '" name="name"></textarea><hr class="main_hr">'
                    )
                else:
                    main_name = load_lang("ban")
                    n_name = (
                        '<input placeholder="'
                        + load_lang("name_or_ip_or_regex")
                        + '" value="'
                        + (name if name else "")
                        + '" name="name"><hr class="main_hr">'
                    )

                regex = (
                    '<input type="checkbox" name="regex" '
                    + ("checked" if ban_type == "regex" else "")
                    + "> "
                    + load_lang("regex")
                    + '<hr class="main_hr">'
                )
                ...
                data = (
                    ...
                    <hr class="main_hr">
                    <input placeholder="'''
                    + load_lang("ban_period")
                    + """ ("""
                    + load_lang("second")
                    + ''')" name="second" id="second" type="text">
                    <hr class="main_hr">
                    <input placeholder="'''
                    + load_lang("why")
                    + """" name="why" type="text">
