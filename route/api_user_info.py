        ...
        if flask.request.method == "POST":
            ...
            for user_name in data_list:
                ...
                if db_data:
                    if db_data[0][0] != "user":
                        curs.execute(
                            db_change("select name from alist where name = ?"),
                            [db_data[0][0]],
                        )
                        if curs.fetchall():
                ...
                if ban_check(name) == 0:
                    ...
                    curs.execute(
                        db_change(
                            "select login, block, end, why from rb where band = 'regex' and ongoing = '1'"
                        )
                    )
                    ...
                    if regex_ban == 0:
                        curs.execute(
                            db_change(
                                "select login, block, end, why from rb where block = ? and ongoing = '1'"
                            ),
                            [user_name],
                        )
                        db_data = curs.fetchall()
