...
def give_auth(name):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change("select data from user_set where id = ? and name = 'acl'"), [name]
        )
        ...
        if flask.request.method == "POST":
            ...
            curs.execute(
                db_change("update user_set set data = ? where id = ? and name = 'acl'"),
                [select_data, name],
            )
            ...
            for data in curs.fetchall():
                if user_acl == data[0]:
                    div = (
                        '<option value="' + data[0] + '">' + data[0] + "</option>" + div
                    )
                else:
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    imp=[
                        ...
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("authorize") + ")", 0]),
                    ],
                    data="""
                        <form method="post">
                            <select name="select">"""
