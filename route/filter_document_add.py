    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            curs.execute(
                ...
                [post_name, "document", post_regex, post_acl],
            )
            ...
            curs.execute(
                db_change(
                    "select plus, plus_t from html_filter where html = ? and kind = 'document'"
                ),
                [name],
            )
            db_data = curs.fetchall()
            acl_list = [
                ["selected" if db_data and db_data[0][1] == i else "", i]
                for i in acl_list
            ]
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    + '''
                            <hr class="main_hr">
                            <input value="'''
                    + (html.escape(db_data[0][0]) if db_data else "")
