    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            if tool == "" or tool == "preview":
                ...
                data_pas = render_set(
                    doc_name=name, doc_data=data_org, data_type="api_view", data_in=""
                )

                return flask.jsonify({"data": data_pas[0], "js_data": data_pas[1]})
            elif tool == "include":
                ...
                try:
                    include_list = json.loads(
                        flask.request.form.get("include_list", "")
                    )
                except:
                    include_list = []
                ...
                if not sql_data:
                    ...
                    get_all_change_1 = []
                    find_replace_moment = re.findall(
                        r"(@([^=@]+)=([^=@]+)@|@([^=@]+)@)", json_data
                    )
                ...
                for i in title_list:
                    curs.execute(
                        db_change("select title from data where title = ?"), [i]
                    )
                    if curs.fetchall():
                        data_exist[i] = "1"
            ...
            if tool == "" or tool == "view":
                if acl_check(name, "render") != 1:
                    if check_int(rev) == "":
                        ...
                        curs.execute(
                            db_change(
                                "select data from history where title = ? and id = ?"
                            ),
                            [name, rev],
                        )
                    ...
                    if sql_data:
                        data_pas = render_set(
                            doc_name=name, doc_data=sql_data[0][0], data_type="api_view"
                        )

                        return flask.jsonify(
                            {"data": data_pas[0], "js_data": data_pas[1]}
                        )
