    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            if name:
                ...
                curs.execute(
                    db_change(
                        "select id, title from rc where type = ? order by date desc"
                    ),
                    [set_type],
                )
                for i in curs.fetchall():
                    curs.execute(
                        db_change(
                            "select id, title, date, ip, send, leng, hide from history where id = ? and title = ?"
                        ),
                        i,
                    )
                    data_list += curs.fetchall()
                ...
                if tool == "history":
                    ...
                    if set_type == "normal":
                        ...
                        if admin == 1:
                            menu += [
                                [
                                    "history_add/" + url_pas(name),
                                    load_lang("history_add"),
                                ],
                                [
                                    "history_reset/" + url_pas(name),
                                    load_lang("history_reset"),
                                ],
                            ]

                    title = name
                    div += next_fix(
                        "/history/" + url_pas(name) + "?tool=" + set_type + "&num=",
                        num,
                        data_list,
                    )
                else:
                    ...
                    div += next_fix(
                        "/record/" + url_pas(name) + "?num=", num, data_list
                    )
            else:
                div = (
                    ""
