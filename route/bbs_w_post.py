...
def bbs_w_post_make_thread(user_id, date, data, code, color="", blind="", add_style=""):
    ...
    return (
        '''
        <table class="opennamu_comment" style="'''
        + add_style
        + """">
            <tr>
                <td class="opennamu_comment_color_"""
        + color
        + '''">
                    <a href="#thread_shortcut" id="'''
        + code
        + """">#"""
        + code
        + """</a>
...
def bbs_w_post(bbs_num="", post_num="", do_type=""):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                "select set_name, set_data, set_code from bbs_data where set_id = ? and set_code = ?"
            ),
            [bbs_num, post_num],
        )
        ...
        curs.execute(
            db_change(
                'select set_data from bbs_set where set_id = ? and set_name = "bbs_type"'
            ),
            [bbs_num],
        )
        ...
        if not db_data_2:
            ...
            if flask.request.method == "POST" and do_type != "preview":
                ...
                curs.execute(
                    db_change(
                        'select set_code from bbs_data where set_name = "comment" and set_id = ? order by set_code + 0 desc'
                    ),
                    [set_id],
                )
                db_data = curs.fetchall()
                ...
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('comment', ?, ?, ?)"
                    ),
                    [id_data, set_id, data],
                )
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('comment_date', ?, ?, ?)"
                    ),
                    [id_data, set_id, date],
                )
                curs.execute(
                    db_change(
                        "insert into bbs_data (set_name, set_code, set_id, set_data) values ('comment_user_id', ?, ?, ?)"
                    ),
                    [id_data, set_id, ip],
                )
                ...
                if do_type == "preview":
                    ...
                    data_preview = render_set(
                        doc_name="", doc_data=text, data_in="from"
                    )
                else:
                    text = ""
                    data_preview = ""
                ...
                data += bbs_w_post_make_thread(
                    ...
                    temp_dict["date"],
                    render_set(doc_name="", doc_data=temp_dict["data"], data_in="from"),
                    "1",
                    color="green",
                )
                ...
                curs.execute(
                    db_change(
                        'select set_name, set_data, set_code, set_id from bbs_data where (set_name = "comment" or set_name = "comment_date" or set_name = "comment_user_id") and set_id = ? order by set_code + 0 asc'
                    ),
                    [bbs_num_str + "-" + post_num_str],
                )
                db_data = curs.fetchall()
                db_data = list(db_data) if db_data else []
                ...
                if bbs_comment_acl == 0:
                    bbs_comment_form = (
                        """                        
                        <textarea name="content" id="opennamu_edit_textarea" class="opennamu_textarea_100">"""
                        + html.escape(text)
                        + """</textarea>
                        <hr class="main_hr">
                        ...
                        <button id="opennamu_save_button" formaction="/bbs/w/"""
                        + bbs_num_str
                        + """/"""
                        + post_num_str
                        + """" type="submit">"""
                        + load_lang("send")
                        + """</button>
                        <button id="opennamu_preview_button" formaction="/bbs/w/preview/"""
                        + bbs_num_str
                        + """/"""
                        + post_num_str
                        + """#opennamu_edit_textarea" type="submit">"""
                        + load_lang("preview")
                        + """</button>
                        <hr class="main_hr">
                    """
                ...
                return easy_minify(
                    flask.render_template(
                        ...
                        data=data,
                        menu=[
                            ["bbs/w/" + bbs_num_str, load_lang("return")],
                            [
                                "bbs/edit/" + bbs_num_str + "/" + post_num_str,
                                load_lang("edit"),
                            ],
                        ],
                    )
            ...
            if flask.request.method == "POST" and do_type != "preview":
                ...
                if select != "":
                    ...
                    if len(select) < 2:
                        curs.execute(
                            db_change(
                                'select set_code from bbs_data where set_name = "comment" and set_id = ? and set_code = ? limit 1'
                            ),
                            [bbs_num_str + "-" + post_num_str, select[0]],
                        )
                ...
                comment_select += (
                    '<option value="default">' + load_lang("normal") + "</option>"
                )
                ...
                while for_a < db_data_len:
                    if temp_id != (db_data_2[for_a][3] + "-" + db_data_2[for_a][2]):
                        if temp_id != "":
                            ...
                            temp_dict["code"] = re.sub(
                                r"^[0-9]+-[0-9]+-", "", temp_dict["code"]
                            )
                            ...
                            if db_data:
                                db_data_2 = (
                                    db_data_2[:for_a] + db_data + db_data_2[for_a:]
                                )
