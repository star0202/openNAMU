        ...
        if flask.request.method == "POST":
            ...
            for for_b in set_list:
                if for_b in use_cookie:
                    html_data.set_cookie(
                        for_b, flask.request.form.get(for_b, set_list[for_b][0][0])
                    )
                    ...
                    flask.session[for_b] = flask.request.form.get(
                        for_b, set_list[for_b][0][0]
                    )
            ...
            for for_b in set_list:
                ...
                for for_a in set_list[for_b]:
                    if get_data == for_a[0]:
                        ...
                        set_data[for_b] += (
                            '<option value="' + for_a[0] + '">' + for_a[1] + "</option>"
                        )

            set_data_main = {}
            for for_b in set_list:
                curs.execute(
                    db_change("select data from other where name = ?"), [for_b]
                )
                db_data = curs.fetchall()
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    <form method="post">
                        ...
                        + """</h3>
                        <sup>"""
                        + load_lang("only_korean")
                        + """</sup> <sup>"""
                        + load_lang("unavailable_in_monaco")
                        + """</sup>
                        <hr class="main_hr">
                        """
                ...
                """
                    ...
                    menu=[
                        ...
                        ["change/skin_set", load_lang("skin_set")],
                    ],
                )
            )
