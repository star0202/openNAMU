...
def topic_list(name="Test"):
    with get_db_connect() as conn:
        ...
        if tool == "close":
            curs.execute(
                db_change(
                    "select code, sub from rd where title = ? and stop = 'O' order by sub asc"
                ),
                [name],
            )
            ...
            curs.execute(
                db_change(
                    "select code, sub from rd where title = ? and agree = 'O' order by sub asc"
                ),
                [name],
            )
            ...
            plus = (
                ...
                + url_pas(name)
                + '?tool=close">('
                + load_lang("closed_discussion")
                + """)</a>
                <a href="/topic/"""
                + url_pas(name)
                + '?tool=agree">('
                + load_lang("agreed_discussion")
                + """)</a>
                <hr class="main_hr">
                <a href="/thread/0/"""
                + url_pas(name)
                + """">("""
                + load_lang("make_new_topic")
                + """)</a>
            """
            )
            ...
            curs.execute(
                db_change(
                    "select code, sub from rd where title = ? and stop != 'O' order by date desc"
                ),
                [name],
            )
        ...
        for data in curs.fetchall():
            ...
            div += (
                ...
                + html.escape(data[1])
                + """</a></h2>
                <div class="topic_pre" id="opennamu_thread_"""
                + data[0]
                + """"></div>
                <div class="topic_back_pre" id="opennamu_thread_back_"""
                + data[0]
                + """"></div>
