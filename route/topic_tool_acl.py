    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            if topic_check:
                ...
                curs.execute(
                    db_change("update rd set acl = ? where code = ?"),
                    [acl_data, topic_num],
                )
                ...
                if db_data:
                    curs.execute(
                        db_change(
                            "update topic_set set set_data = ? where thread_code = ?"
                        ),
                        [acl_data_view, topic_num],
                    )
                else:
                    curs.execute(
                        db_change(
                            "insert into topic_set (thread_code, set_name, set_id, set_data) values (?, 'thread_view_acl', '1', ?)"
                        ),
                        [topic_num, acl_data_view],
                    )

                do_add_thread(
                    topic_num, load_lang("acl_thread_change") + " : " + acl_data, "1"
                )
