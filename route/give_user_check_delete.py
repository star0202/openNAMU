...
def give_user_check_delete_2():
    with get_db_connect() as conn:
        ...
        time_set = re.search(
            r"([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})", time
        )
        if not time_set:
            return redirect()
        ...
        if user_id and user_ip and time:
            if flask.request.method == "POST":
                curs.execute(
                    db_change(
                        "delete from ua_d where name = ? and ip = ? and today = ?"
                    ),
                ...
                return redirect(
                    "/check/" + url_pas(user_id if return_type == "0" else user_ip)
                )
            else:
                return easy_minify(
                    ...
                    """,
                        menu=[
                            [
                                "check/"
                                + url_pas(user_id if return_type == "0" else user_ip),
                                load_lang("return"),
                            ]
                        ],
                    )
