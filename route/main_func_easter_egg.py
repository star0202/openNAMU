...
def main_func_easter_egg():
    with get_db_connect() as conn:
        ...
        if ip_or_user(ip) == 0:
            curs.execute(
                db_change("select name from user_set where id = ? and name = ?"),
                [ip, "get_🥚"],
            )
            if not curs.fetchall():
                curs.execute(
                    db_change(
                        'insert into user_set (name, id, data) values ("get_🥚", ?, "Y")'
                    ),
                    [ip],
                )
                conn.commit()
        ...
        return easy_minify(
            flask.render_template(
                ...
                data='<iframe width="640" height="360" src="https://www.youtube.com/embed/'
                + select_list[random_n]
