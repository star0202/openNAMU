...
def main_setting_acl():
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            if admin_check(None, "edit_set (acl)") != 1:
                ...
                for i in i_list:
                    curs.execute(
                        ...
                        [flask.request.form.get(i_list[i], "normal"), i_list[i]],
                    )
            ...
            for i in i_list:
                curs.execute(
                    db_change("select data from other where name = ?"), [i_list[i]]
                )
                sql_d = curs.fetchall()
                if sql_d:
                    d_list[i] = sql_d[0][0]
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    <form method="post">
                        ...
                        <select """
                        + disable
                        + """ name="edit_bottom_compulsion_acl">"""
                        + acl_div[10]
                        + """</select>
