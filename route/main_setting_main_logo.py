...
def main_setting_main_logo():
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            for i in i_list:
                curs.execute(
                    db_change("select data from other where name = ? and coverage = ?"),
                    [i[0], i[1]],
                )
                sql_d = curs.fetchall()
                if sql_d:
            ...
            for i in range(0, len(skin_list)):
                end_data += (
                    ...
                    + load_lang("wiki_logo")
                    + " "
                    + ("(" + skin_list[i] + ")" if skin_list[i] != 0 else "")
                    + " (HTML)"
                    + '<hr class="main_hr">'
                    + '<input name="'
                    + (skin_list[i] if skin_list[i] != 0 else "main_css")
                    + '" value="'
                    + html.escape(d_list[i])
                    + '">'
                    + '<hr class="main_hr">'
                    + ""
                )
