...
def user_watch_list_name(tool, name="Test"):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                "select title from scan where user = ? and title = ? and type = ?"
            ),
            [ip, name, type_data],
        )
        if curs.fetchall():
            curs.execute(
                db_change("delete from scan where user = ? and title = ? and type = ?"),
                [ip, name, type_data],
            )
        else:
            curs.execute(
                db_change("insert into scan (user, title, type) values (?, ?, ?)"),
                [ip, name, type_data],
            )
