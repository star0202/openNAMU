        ...
        if flask.request.method == "POST":
            ...
            return easy_minify(
                flask.render_template(
                    skin_check(),
                    imp=[
                        load_lang("sitemap_management"),
                        wiki_set(),
                        wiki_custom(),
                        wiki_css(["(" + load_lang("beta") + ")", 0]),
                    ],
                    data="""
                    """
                    + sitemap_list
                    ...
                    <hr class="main_hr">
                    <form method="post">
                        <input type="checkbox" name="exclude_domain"> """
                    + load_lang("stiemap_exclude_domain")
                    + """
                        <hr class="main_hr">

                        <input type="checkbox" name="exclude_user_page"> """
                    + load_lang("stiemap_exclude_user_page")
                    + """
                        <hr class="main_hr">

                        <input type="checkbox" name="exclude_file_page"> """
                    + load_lang("stiemap_exclude_file_page")
                    + """
                        <hr class="main_hr">

                        <input type="checkbox" name="exclude_category_page"> """
                    + load_lang("stiemap_exclude_category_page")
                    + """
                        <hr class="main_hr">
