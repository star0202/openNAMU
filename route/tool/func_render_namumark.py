...
class class_do_render_namumark:
    def __init__(self, curs, doc_name, doc_data, doc_set, lang_data):
        ...
        self.doc_include = (
            self.doc_set["doc_include"] if "doc_include" in self.doc_set else ""
        )
        ...
        for for_a in self.data_footnote:
            ...
            if len(self.data_footnote[for_a]["list"]) > 1:
                ...
                data += (
                    ...
                    + self.doc_include
                    + "fn_"
                    + self.data_footnote[for_a]["list"][0]
                    + '" href="#'
                    + self.doc_include
                    + "rfn_"
                    + self.data_footnote[for_a]["list"][0]
    ...
    def get_tool_link_fix(self, link_main, do_type="link"):
        if do_type == "link":
            if link_main == "../":
                ...
                link_main = re.sub(
                    r"^:(분류|category):", "category:", link_main, flags=re.I
                )
    ...
    def do_render_text(self):
        # <b> function
        bold_user_set = get_main_skin_set(
            self.curs, self.flask_session, "main_css_bold", self.ip
        )
        ...
        # <u>
        self.render_data = re.sub(
            r"__((?:(?!__).)+)__", do_render_text_under, self.render_data
        )
        ...
        # <sup>
        self.render_data = re.sub(
            r"\^\^\^((?:(?!\^\^\^).)+)\^\^\^", do_render_text_sup, self.render_data
        )
        # <sup> 2
        self.render_data = re.sub(
            r"\^\^((?:(?!\^\^).)+)\^\^", do_render_text_sup, self.render_data
        ...
        # <sub>
        self.render_data = re.sub(
            r",,,((?:(?!,,,).)+),,,", do_render_text_sub, self.render_data
        )
        # <sub> 2
        self.render_data = re.sub(
            r",,((?:(?!,,).)+),,", do_render_text_sub, self.render_data
        ...
        # <s> function
        strike_user_set = get_main_skin_set(
            self.curs, self.flask_session, "main_css_strike", self.ip
        )
        ...
        # <s>
        self.render_data = re.sub(
            r"--((?:(?!--).)+)--", do_render_text_strike, self.render_data
        )
        # <s> 2
        self.render_data = re.sub(
            r"~~((?:(?!~~).)+)~~", do_render_text_strike, self.render_data
    ...
    def do_render_heading(self):
            ...
            if not re.search(heading_regex, self.render_data):
                ...
                if heading_data_diff != heading_data_last:
                    ...
                    for for_a in reversed(range(1, 7)):
                        ...
                        heading_restore_regex = re.compile(
                            "^={" + for_a_str + "}|={" + for_a_str + "}$"
                        )

                        heading_data_all = re.sub(
                            heading_restore_regex,
                            "<heading_" + for_a_str + ">",
                            heading_data_all,
                        )
                    ...
                    heading_stack_str = ".".join(
                        [str(for_a) for for_a in heading_stack]
                    )
                    ...
                    self.render_data = re.sub(
                        heading_regex, heading_data_complete, self.render_data, 1
                    )
        ...
        for for_a in reversed(range(0, 6)):
            ...
            heading_end_match_count = len(
                re.findall(heading_end_stack_regex, self.render_data)
            )
            if heading_end_match_count == heading_end_count:
                self.render_data = re.sub(
                    heading_end_stack_regex, "<heading_stack>", self.render_data
                )
        ...
        for for_a in range(1, 7):
            ...
            self.render_data = re.sub(
                heading_restore_regex, ("=" * for_a), self.render_data
            )
    ...
    def do_render_macro(self):
        ...
        def do_render_macro_double(match):
            ...
            if name_data in ("youtube", "nicovideo", "navertv", "kakaotv", "vimeo"):
                ...
                for for_a in data:
                    ...
                    if data_sub:
                        ...
                        if data_sub[0] == "width":
                            ...
                            data_sub[0] == "https://www.youtube.com/watch?v"
                            and name_data == "youtube"
                        ):
                ...
                if name_data == "youtube":
                    ...
                    video_code = re.sub(
                        r"^https:\/\/tv\.kakao\.com\/v\/", "", video_code
                    )
                    ...
                    video_code = re.sub(
                        r"^https:\/\/tv\.naver\.com\/v\/", "", video_code
                    )
                ...
                if color != "":
                    sub_text = (
                        '<span style="color:' + color + ';">' + sub_text + "</span>"
                    )
                ...
                data_name = self.get_tool_data_storage(
                    '<span id="' + main_text + '">', "</span>", match_org.group(0)
                )
                ...
                self.curs.execute(
                    db_change(
                        "select set_data from data_set where doc_name = ? and set_name = 'last_edit'"
                    ),
                    [link_main],
                )
        ...
        self.render_data = re.sub(
            r"\[([^[(]+)\(([^()]+)\)\]", do_render_macro_double, self.render_data
        )
        ...
        def do_render_macro_single(match):
            ...
            if match in ("date", "datetime"):
                ...
                data_name = self.get_tool_data_storage(
                    '<div style="clear: both;"></div>', "", match_org.group(0)
                )
                ...
                self.curs.execute(
                    db_change('select data from other where name = "count_all_title"')
                )
                db_data = self.curs.fetchall()
                if db_data:
                    return db_data[0][0]
        ...
        self.render_data = re.sub(
            r"\[([^[\]]+)\]", do_render_macro_single, self.render_data
        )
    ...
    def do_render_math(self):
        def do_render_math_sub(match):
            ...
            data_name = self.get_tool_data_storage(
                '<span id="' + name_ob + '">' + data_html, "</span>", match.group(0)
            )
        ...
        math_regex_2 = re.compile(
            "&lt;math&gt;((?:(?!&lt;math&gt;|&lt;\/math&gt;).)+)&lt;\/math&gt;", re.I
        )
            ...
            if not re.search(link_regex, self.render_data):
                ...
                if re.search(r"^(파일|file|외부|out):", link_main, flags=re.I):
                        ...
                        for for_a in data:
                            ...
                            if data_sub:
                                ...
                                if data_sub[0] == "width":
                                    ...
                                    file_height = self.get_tool_px_add_check(
                                        data_sub[1]
                                    )
                                    ...
                                    file_radius = self.get_tool_px_add_check(
                                        data_sub[1]
                                    )
                    ...
                    if re.search(link_out_regex, link_main):
                        ...
                        link_main = self.get_tool_data_restore(
                            link_main, do_type="slash"
                        )
                        link_main = html.unescape(link_main)
                        link_main = re.sub(r'"', "&quot;", link_main)
                        ...
                        self.curs.execute(
                            db_change("select title from data where title = ?"),
                            ["file:" + link_main],
                        )
                        db_data = self.curs.fetchall()
                        if db_data:
                            link_exist = ""
                            self.data_backlink += [
                                [self.doc_name, "file:" + link_main, "file"]
                            ]
                        else:
                            link_exist = "opennamu_not_exist_link"
                            self.data_backlink += [
                                [self.doc_name, "file:" + link_main, "no"]
                            ]
                            self.data_backlink += [
                                [self.doc_name, "file:" + link_main, "file"]
                            ]
                        ...
                        link_main = (
                            "/image/"
                            + url_pas(sha224_replace(link_main))
                            + "."
                            + link_extension
                        )
                    ...
                    if file_height != "":
                        file_height = (
                            "height:" + self.get_tool_css_safe(file_height) + ";"
                        )
                    ...
                    if file_bgcolor != "":
                        file_bgcolor = (
                            "background:" + self.get_tool_css_safe(file_bgcolor) + ";"
                        )

                    if file_radius != "":
                        file_radius = (
                            "border-radius:" + self.get_tool_css_safe(file_radius) + ";"
                        )

                    image_set = get_main_skin_set(
                        self.curs, self.flask_session, "main_css_image_set", self.ip
                    )
                    ...
                    if file_align == "center":
                        file_end = (
                            '<div style="text-align:center;">' + file_end + "</div>"
                        )
                        ...
                        if file_pass == 1:
                            ...
                            if image_set == "new_click":
                                ...
                                self.render_data_js += (
                                    ...
                                    + str(image_count)
                                    + '''").src = "'''
                                    + self.get_tool_js_safe(link_main)
                                    + """";
                                        setTimeout(function() {
                                            document.getElementById("opennamu_image_"""
                                    + str(image_count)
                                    + '''_link").href = "'''
                                    + self.get_tool_js_safe(file_link)
                                    + """";
                                        }, 100);
                                    });\n
                                """
                ...
                # category
                    ...
                    if not link_main in self.data_category_list:
                        ...
                        if db_data:
                            link_exist = ""
                            self.data_backlink += [
                                [self.doc_name, "category:" + link_main, "cat"]
                            ]
                        else:
                            link_exist = "opennamu_not_exist_link"
                            self.data_backlink += [
                                [self.doc_name, "category:" + link_main, "no"]
                            ]
                            self.data_backlink += [
                                [self.doc_name, "category:" + link_main, "cat"]
                            ]
                ...
                # inter link
                    ...
                    self.curs.execute(
                        db_change(
                            "select plus, plus_t from html_filter where kind = 'inter_wiki' and html = ?"
                        ),
                        [link_inter_name],
                    )
                    db_data = self.curs.fetchall()
                    if db_data:
                        ...
                        if link_data[1]:
                            ...
                            link_sub_storage = re.sub(
                                link_inter_regex, "", link_sub_storage
                            )
                        ...
                        data_name = self.get_tool_data_storage(
                            '<a class="opennamu_link_inter" title="'
                            + link_title
                            + '" href="'
                            + link_main
                            + link_data_sharp
                ...
                # out link
                    ...
                    data_name = self.get_tool_data_storage(
                        '<a class="opennamu_link_out" target="_blank" title="'
                        + link_title
                        + '" href="'
                        + link_main
                        + '">'
    ...
    def do_render_include_default(self):
        ...
        self.render_data = re.sub(
            r"(\\+)?@([ㄱ-힣a-zA-Z]+)=((?:\\@|[^@\n])+)@",
            do_render_include_default_sub,
            self.render_data,
        )
        self.render_data = re.sub(
            r"(\\+)?@([ㄱ-힣a-zA-Z]+)@", do_render_include_default_sub, self.render_data
        )
    ...
    def do_render_include(self):
        ...
        include_num = 0
        include_set_data = get_main_skin_set(
            self.curs, self.flask_session, "main_css_include_link", self.ip
        )
        include_regex = re.compile(
            "\[include\(((?:(?!\[include\(|\)\]|<\/div>).)+)\)\]", re.I
        )
                ...
                if re.search("opennamu_include_", self.doc_include):
                    ...
                    for for_a in data:
                        ...
                        if data_sub:
                            ...
                            data_sub_data = self.get_tool_data_restore(
                                data_sub[1], do_type="slash"
                            )

                            data_sub_data = re.sub(
                                r"^(?P<in>분류|category):", ":\g<in>:", data_sub_data
                            )
                            data_sub_data = re.sub(
                                r"^(?P<in>파일|file):", ":\g<in>:", data_sub_data
                            )
                    ...
                    include_name = self.get_tool_data_restore(
                        include_name, do_type="slash"
                    )
                    include_name = html.unescape(include_name)

                    # load include db data
                    self.curs.execute(
                        db_change("select data from data where title = ?"),
                        [include_name],
                    )
                    db_data = self.curs.fetchall()
                    if db_data:
                        ...
                        include_link = (
                            '<div><a class="opennamu_not_exist_link" href="/w/'
                            + url_pas(include_name)
                            + '">('
                            + include_name_org
                            + ")</a></div>"
    ...
    def do_redner_footnote(self):
        footnote_num = 0
        footnote_set = get_main_skin_set(
            self.curs, self.flask_session, "main_css_footnote_set", self.ip
        )
        ...
        while 1:
            ...
            if footnote_count_all < 0:
                ...
                if footnote_data[2]:
                    self.render_data = re.sub(
                        footnote_regex,
                        lambda x: self.get_tool_footnote_make(),
                        self.render_data,
                        1,
                    )
                else:
                    ...
                    if footnote_set == "spread":
                        ...
                        self.render_data_js += (
                            'document.getElementById("'
                            + rfn
                            + '_over").addEventListener("mouseenter", function() { opennamu_do_footnote_popover("'
                            + rfn
                            + '", "'
                            + fn
                            + '"); });\n'
                        ...
                        self.render_data_js += (
                            'document.getElementById("'
                            + rfn
                            + '_over").addEventListener("mouseleave", function() { opennamu_do_footnote_popover("'
                            + rfn
                            + '", "'
                            + fn
                            + '"); });\n'
    ...
    def do_render_redirect(self):
        ...
        if match and self.doc_include == "":
            ...
            if "doc_from" in self.doc_set:
                data_name = self.get_tool_data_storage(
                    '<a href="' + link_main + link_data_sharp + '">(GO)</a>',
                    "",
                    link_data_full,
                )
            else:
        ...
        def do_render_table_parameter(cell_count, parameter, data, option={}):
            ...
            for table_parameter in re.findall(table_parameter_regex, parameter):
                ...
                if len(table_parameter_split) == 2:
                    ...
                    table_parameter_data = self.get_tool_css_safe(
                        table_parameter_split[1]
                    )

                    if table_parameter_name == "tablebgcolor":
                        table_parameter_all["table"] += (
                            "background:"
                            + self.get_tool_dark_mode_split(table_parameter_data)
                            + ";"
                        )
                    elif table_parameter_name == "tablewidth":
                        table_parameter_all["div"] += (
                            "width:"
                            + self.get_tool_px_add_check(table_parameter_data)
                            + ";"
                        )
                        ...
                        table_parameter_all["table"] += (
                            "text-align:" + table_parameter_data + ";"
                        )
                    ...
                    if re.search(r"^-[0-9]+$", table_parameter):
                        ...
                        table_parameter_all["colspan"] = re.sub(
                            r"[^0-9]+", "", table_parameter
                        )
                        ...
                        table_parameter_all["rowspan"] = re.sub(
                            r"[^0-9]+", "", table_parameter
                        )
        ...
        while 1:
            ...
            if table_count_all < 0:
                ...
                for table_sub in re.findall(table_sub_regex, table_data):
                    ...
                    table_sub_parameter = do_render_table_parameter(
                        table_sub[1], table_sub[2], table_data_in
                    )

                    if table_data_end == "":
                        table_data_end += (
                            '<tr style="' + table_sub_parameter["tr"] + '">'
                        )

                    if table_sub[0] != "" and table_tr_change == 1:
                        table_col_num = 0
                        table_data_end += (
                            '</tr><tr style="' + table_sub_parameter["tr"] + '">'
                        )
                    ...
                    table_parameter["class"] = (
                        table_sub_parameter["class"]
                        if table_sub_parameter["class"] != ""
                        else table_parameter["class"]
                    )
                    ...
                    if table_sub[2] == "" and table_sub[3] == "":
                        ...
                        table_data_end += (
                            '<td colspan="'
                            + table_sub_parameter["colspan"]
                            + '" rowspan="'
                            + table_sub_parameter["rowspan"]
                            + '" style="'
                            + table_parameter["col"][table_col_num]
                            + table_sub_parameter["td"]
                            + '"><back_br>\n'
    ...
    def do_render_middle(self):
            ...
            if middle_count_all < 0:
                ...
                if middle_slash:
                    if self.data_temp_storage[middle_slash] != "}":
                        middle_data_org = re.sub(
                            r"<(\/?(?:slash)_(?:[0-9]+))>",
                            "<temp_" + middle_slash + ">",
                            middle_data_org,
                        )
                        self.render_data = re.sub(
                            middle_regex, lambda x: middle_data_org, self.render_data, 1
                        )
                        continue
                ...
                if middle_name:
                    ...
                    if middle_name == "#!wiki":
                        if middle_slash:
                            ...
                            self.render_data = re.sub(
                                middle_regex,
                                lambda x: middle_data_org,
                                self.render_data,
                                1,
                            )
                            continue

                        wiki_regex = re.compile(
                            "^#!wiki(?:(?: style=(&quot;(?:(?:(?!&quot;).)*)&quot;|&#x27;(?:(?:(?!&#x27;).)*)&#x27;))| [^\n]*)?\n",
                            re.I,
                        )
                        ...
                        self.data_include += [
                            [
                                self.doc_include + "opennamu_wiki_" + str(wiki_count),
                                self.doc_name,
                                wiki_data,
                                wiki_data_style,
                            ]
                        ...
                        data_name = self.get_tool_data_storage(
                            '<span style="font-size:' + wiki_size + '%">',
                            "</span>",
                            middle_data_org,
                        )
                    elif middle_name in ("-5", "-4", "-3", "-2", "-1"):
                        ...
                        r"^@(?:((?:[0-9a-f-A-F]{3}){1,2})|(\w+))", middle_name
                    ):
                        if middle_slash:
                            middle_data_org = re.sub(
                                r"<(\/?(?:slash)_(?:[0-9]+))>",
                        ...
                        wiki_color = re.search(
                            r"^@(?:((?:[0-9a-f-A-F]{3}){1,2})|(\w+))(,@(?:((?:[0-9a-f-A-F]{3}){1,2})|(\w+)))?",
                            middle_name,
                        )
                        wiki_color_data = ""
                        if wiki_color:
                        ...
                        middle_data_pass = wiki_data
                        data_name = self.get_tool_data_storage(
                            '<span style="background-color:' + wiki_color + '">',
                            "</span>",
                            middle_data_org,
                        )
                    elif re.search(
        ...
        self.render_data = re.sub(
            r"<temp_(?P<in>(?:slash)_(?:[0-9]+))>", "<\g<in>>", self.render_data
        )
    ...
    def do_render_list(self):
        ...
        while 1:
            ...
            if quote_count_max < 0:
                ...
                self.data_include += [
                    [
                        self.doc_include + "opennamu_quote_" + str(quote_count),
                        self.doc_name,
                        quote_data,
                        "",
                    ]
    ...
    def do_render_last(self):
        ...
        if self.doc_include == "":
            if self.data_category != "":
                ...
                category_set_data = get_main_skin_set(
                    self.curs, self.flask_session, "main_css_category_set", self.ip
                )
                if category_set_data == "bottom":
                    if re.search(r"<footnote_category>", self.render_data):
                        self.render_data = re.sub(
                            r"<footnote_category>",
                            "<hr><" + data_name + "></" + data_name + ">",
                            self.render_data,
                            1,
                        )
                    else:
                    ...
                    self.render_data = re.sub(
                        r"<footnote_category>", "", self.render_data, 1
                    )
                    self.render_data = (
                        "<"
                        + data_name
        ...
        self.render_data = re.sub(
            r"(<a(?: [^<>]*)?>|<\/a>)", do_render_last_a_link, self.render_data
        )
        ...
        def do_render_last_toc(match):
            ...
            if heading_data:
                ...
                self.render_data = re.sub(
                    heading_regex,
                    lambda x: ("<h" + heading_data + ' id="' + data + '">'),
                    self.render_data,
                    1,
                )
        ...
        if self.data_toc != "":
            ...
            toc_search_regex = (
                r"<toc_data>((?:(?!<toc_data>|<\/toc_data>).)*)<\/toc_data>"
            )
            ...
            self.data_toc = toc_data
            self.data_toc = re.sub(
                r"<toc_inside>((?:(?!<toc_inside>|<\/toc_inside>).)*)<\/toc_inside>",
                do_render_last_toc,
                self.data_toc,
            )

            toc_set_data = get_main_skin_set(
                self.curs, self.flask_session, "main_css_toc_set", self.ip
            )
            ...
            if toc_set_data == "off":
                ...
                self.render_data = re.sub(
                    r"<toc_need_part>", lambda x: (self.data_toc), self.render_data, 20
                )
                self.render_data = re.sub(r"<toc_need_part>", "", self.render_data)

            if (
                self.doc_include != ""
                or re.search(r"<toc_no_auto>", self.render_data)
                or toc_set_data == "half_off"
                or toc_set_data == "off"
                or toc_data_on == 1
            ):
        ...
        def do_render_last_footnote(match):
            ...
            find_regex = re.compile(
                '<footnote_title id="'
                + match
                + '_title">((?:(?!<footnote_title|<\/footnote_title>).)*)<\/footnote_title>'
            )
        ...
        self.render_data = re.sub(
            r'<a fn_target="([^"]+)"', do_render_last_footnote, self.render_data
        )
