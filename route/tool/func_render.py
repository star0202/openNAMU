...
class class_do_render:
    def __init__(self, conn, lang_data={}):
        ...
        if lang_data == "{}":
            lang_data = {"toc": "toc", "category": "category"}
    ...
    def do_render(self, doc_name, doc_data, data_type, data_in):
        ...
        if rep_data == "namumark" or rep_data == "namumark_beta":
            data_end = class_do_render_namumark(
                curs, doc_name, doc_data, doc_set, self.lang_data
            )()
        elif rep_data == "raw":
        ...
        if data_type == "backlink":
            ...
            if backlink != []:
                curs.executemany(
                    db_change("insert into back (link, title, type) values (?, ?, ?)"),
                    data_end[2]["backlink"],
                )
                curs.execute(
                    db_change("delete from back where title = ? and type = 'no'"),
                    [doc_name],
                )
