...
if data_up_date == 1:
    ...
    if platform.system() in ("Linux", "Windows"):
        if platform.python_implementation() == "PyPy":
            os.system(
                "pypy"
                + ("3" if platform.system() != "Windows" else "")
                + " "
...
def get_init_set_list(need="all"):
    init_set_list = {
        "host": {"display": "Host", "require": "conv", "default": "0.0.0.0"},
        "port": {"display": "Port", "require": "conv", "default": "3000"},
        "language": {
            "display": "Language",
            "require": "select",
            "default": "ko-KR",
...
class class_check_json:
    def do_check_set_json():
        if os.getenv("NAMU_DB") or os.getenv("NAMU_DB_TYPE"):
            ...
            set_data["db_type"] = (
                os.getenv("NAMU_DB_TYPE") if os.getenv("NAMU_DB_TYPE") else "sqlite"
            )
        else:
            if os.path.exists(os.path.join("data", "set.json")):
                db_set_list = ["db", "db_type"]
                with open(
                    os.path.join("data", "set.json"), encoding="utf8"
                ) as file_data:
...
def update(ver_num, set_data):
    ...
    if ver_num < 3170400:
        ...
        for i in curs.fetchall():
            curs.execute(
                db_change("update topic set code = ? where title = ? and sub = ?"),
                [i[2], i[0], i[1]],
            )
            curs.execute(
                db_change("update rd set code = ? where title = ? and sub = ?"),
                [i[2], i[0], i[1]],
            )
    ...
    if ver_num < 3171800:
        ...
        if change_rec and change_rec[0][0] != "":
            ...
            if new_rec:
                curs.execute(
                    db_change("update other set data = ? where name = 'recaptcha'"),
                    [new_rec.group(1)],
                )
            else:
                curs.execute(
                ...
                curs.execute(
                    db_change("update other set data = '' where name = 'sec_re'")
                )
    ...
    if ver_num < 3190201:
        ...
        for i in curs.fetchall():
            curs.execute(
                ...
                [i[0], i[1], today_time, i[2], i[3], i[4], "1"],
            )
    ...
    if ver_num < 3202400:
        ...
        if get_data and get_data[0][0] == "master":
            curs.execute(
                db_change("update other set data = 'beta' where name = 'update'"), []
            )
    ...
    if ver_num < 3205500:
        ...
        for i in curs.fetchall():
            curs.execute(
                db_change("insert into acl (title, data, type) values (?, ?, ?)"),
                [i[0], i[1], "decu"],
            )
            curs.execute(
                db_change("insert into acl (title, data, type) values (?, ?, ?)"),
    ...
    if ver_num < 3302302:
        ...
        for i in curs.fetchall():
            curs.execute(
                db_change("insert into user_set (name, id, data) values (?, ?, ?)"),
                ["pw", i[0], i[1]],
            )
            curs.execute(
                db_change("insert into user_set (name, id, data) values (?, ?, ?)"),
    ...
    if ver_num < 3500106:
        ...
        if db_data and db_data[0][0] != "":
            ...
            if db_data:
                ...
                curs.execute(
                    db_change("update other set data = ? where name = 'domain'"),
                    [db_data],
                )
            else:
                curs.execute(
    ...
    if ver_num < 3500354:
        ...
        if db_data:
            ...
            if db_data[0][0] == robot_default:
                curs.execute(
                    db_change(
                        "insert into other (name, data, coverage) values ('robot_default', 'on', '')"
                    )
                )
    ...
    if ver_num < 3500360:
        ...
        for for_a in db_data:
            curs.execute(
                db_change(
                    "select date from history where title = ? order by date desc limit 1"
                ),
                [for_a[0]],
            )
    ...
    if ver_num < 3500361:
        ...
        for db_data in curs.fetchall():
            if ip_or_user(db_data[0]) == 1:
                curs.execute(
                    db_change('delete from user_set where id = ? and name = "email"'),
                    [db_data[0]],
                )
...
def set_init_always(ver_num):
    ...
    curs.execute(
        db_change('insert into other (name, data, coverage) values ("ver", ?, "")'),
        [ver_num],
    )
    ...
    if not curs.fetchall():
        curs.execute(
            db_change('insert into other (name, data, coverage) values ("key", ?, "")'),
            [load_random_key()],
        )
    ...
    curs.execute(
        db_change('select data from other where name = "wiki_access_password_need"')
    )
    db_data = curs.fetchall()
    if db_data and db_data[0][0] != "":
        curs.execute(
            db_change('select data from other where name = "wiki_access_password"')
        )
        db_data = curs.fetchall()
        if db_data:
...
def set_init():
    ...
    if not curs.fetchall():
        for i in [
            ...
            curs.execute(
                db_change("insert into other (name, data, coverage) values (?, ?, '')"),
                [i[0], i[1]],
            )
...
def get_user_title_list(ip=""):
    ...
    curs.execute(
        db_change("select name from user_set where id = ? and name = ?"), [ip, "get_🥚"]
    )
    if curs.fetchall():
...
def http_warning():
    return (
        ...
        <span style="display: none;" id="opennamu_http_warning_text_lang">"""
...
def load_domain(data_type="normal"):
    ...
    if data_type == "full":
        ...
        db_data = curs.fetchall()
        domain += (
            db_data[0][0] if db_data and db_data[0][0] != "" else flask.request.host
        )
    else:
        curs.execute(db_change("select data from other where name = 'domain'"))
        db_data = curs.fetchall()
...
def edit_button(
    ob_name="opennamu_edit_textarea", monaco_ob_name="opennamu_monaco_editor"
):
    curs = conn.cursor()
    ...
    curs.execute(
        db_change("select html, plus from html_filter where kind = 'edit_top'")
    )
    db_data = curs.fetchall()
...
def ip_warning():
    ...
    if ip_or_user() != 0:
        curs.execute(
            db_change('select data from other where name = "no_login_warning"')
        )
        data = curs.fetchall()
        if data and data[0][0] != "":
            text_data = (
                "" + "<span>" + data[0][0] + "</span>" + '<hr class="main_hr">' + ""
            )
        else:
            text_data = (
                ""
                + "<span>"
                + load_lang("no_login_warning")
                + "</span>"
                + '<hr class="main_hr">'
                + ""
            )
...
def pw_check(data, data2, type_d="no", id_d=""):
    ...
    if db_data[0][0] != set_data and re_data == 1 and id_d != "":
        curs.execute(
            db_change("update user_set set data = ? where id = ? and name = 'pw'"),
            [pw_encode(data), id_d],
        )
        curs.execute(
            db_change("update user_set set data = ? where id = ? and name = 'encode'"),
            [db_data[0][0], id_d],
        )
...
def load_lang(data, safe=0):
    ...
    if ip_or_user(ip) == 0:
        curs.execute(
            db_change('select data from user_set where name = "lang" and id = ?'), [ip]
        )
    ...
    if lang_name in global_lang:
        ...
        if (lang_name + ".json") in lang_list:
            lang = json.loads(
                open(os.path.join("lang", lang_name + ".json"), encoding="utf8").read()
            )
...
def skin_check(set_n=0):
    ...
    if ip_or_user(ip) == 0:
        curs.execute(
            db_change('select data from user_set where name = "skin" and id = ?'), [ip]
        )
...
def wiki_css(data):
    ...
    # Func JS + Defer
    data_css += (
        '<script src="/views/main_css/js/func/func.js?ver='
        + data_css_ver
        + '"></script>'
    )

    data_css += (
        '<script defer src="/views/main_css/js/func/insert_version.js?ver='
        + data_css_ver
        + '"></script>'
    )
    data_css += (
        '<script defer src="/views/main_css/js/func/insert_user_info.js?ver='
        + data_css_ver
        + '"></script>'
    )
    data_css += (
        '<script defer src="/views/main_css/js/func/insert_version_skin.js?ver='
        + data_css_ver
        + '"></script>'
    )
    data_css += (
        '<script defer src="/views/main_css/js/func/insert_http_warning_text.js?ver='
        + data_css_ver
        + '"></script>'
    )

    data_css += (
        '<script defer src="/views/main_css/js/func/ie_end_of_life.js?ver='
        + data_css_ver
        + '"></script>'
    )
    data_css += (
        '<script defer src="/views/main_css/js/func/shortcut.js?ver='
        + data_css_ver
        + '"></script>'
    )
    ...
    # Route JS
    data_css += (
        '<script src="/views/main_css/js/route/editor.js?ver='
        + data_css_ver
        + '"></script>'
    )
    data_css += (
        '<script src="/views/main_css/js/route/render.js?ver='
        + data_css_ver
        + '"></script>'
    )
    ...
    # Main CSS
    data_css += (
        '<link rel="stylesheet" href="/views/main_css/css/main.css?ver='
        + data_css_ver
        + '">'
    )
...
def wiki_set(num=1):
    ...
    if db_data and db_data[0][0] != "":
        ...
        db_data = curs.fetchall()
        data_list += (
            [db_data[0][0]] if db_data and db_data[0][0] != "" else [data_list[0]]
        )
    ...
    curs.execute(
        db_change("select data from user_set where name = '' and id = ?"), [ip]
    )
    db_data_2 = curs.fetchall()
    ...
    if db_data != "":
        ...
        db_data = [
            [db_data[for_a], db_data[for_a + 1]] for for_a in range(0, len(db_data), 2)
        ]
...
def wiki_custom():
    ...
    if ip_or_user(ip) == 0:
        ...
        curs.execute(
            db_change('select data from user_set where name = "email" and id = ?'), [ip]
        )
        email = curs.fetchall()
        ...
        user_head += (
            flask.session["head" + skin_name]
            if "head" + skin_name in flask.session
            else ""
        )

    curs.execute(
        db_change("select title from rd where title = ? and stop = ''"), ["user:" + ip]
    )
...
def render_set(doc_name="", doc_data="", data_type="view", data_in="", doc_acl=""):
    ...
    if doc_acl == 1:
        ...
        if doc_data != 0:
            ...
            get_class_render = class_do_render(conn, render_lang_data).do_render(
                doc_name, doc_data, data_type, data_in
            )
                ...
                while len(get_class_render[2]["include"]) > for_a:
                    ...
                    if acl_result == 0:
                        include_regex = re.compile(
                            '<div id="' + include_data[0] + '"><\\/div>'
                        )
                        if re.search(include_regex, get_class_render[0]):
                            include_data_render = class_do_render(
                                conn, render_lang_data
                            ).do_render(
                                include_data[1],
                                include_data[2],
                                data_type,
                                include_data[0] + data_in,
                            )
                            if len(include_data) > 3:
                                include_data_render[0] = (
                                    '<div id="'
                                    + include_data[0]
                                    + '" '
                                    + include_data[3]
                            ...
                            get_class_render[0] = re.sub(
                                include_regex,
                                include_data_render[0],
                                get_class_render[0],
                            )
                            get_class_render[1] += include_data_render[1]
                            get_class_render[2]["include"] += include_data_render[2][
                                "include"
                            ]
            ...
            curs.execute(
                db_change("select data from other where name = 'namumark_compatible'")
            )
            db_data = curs.fetchall()
            if db_data and db_data[0][0] != "":
                get_class_render[
            ...
            if data_type == "api_view":
                ...
                return (
                    get_class_render[0] + "<script>" + get_class_render[1] + "</script>"
                )
        else:
...
def render_simple_set(data):
    ...
    for toc_search_in in toc_search_data:
        ...
        heading_stack_str = "".join(
            [str(for_a) + "." if for_a != 0 else "" for for_a in heading_stack]
        )
...
def get_admin_list(num=None):
    ...
    if num == "all":
        curs.execute(
            db_change("select data from user_set where data != 'user' and name = 'acl'")
        )
        db_data = curs.fetchall()
        ...
        for db_data_in in db_data:
            curs.execute(
                db_change("select id from user_set where data = ? and name = 'acl'"),
                [db_data_in[0]],
            )
            db_data_2 = curs.fetchall()
            admin_list += (
                [db_data_2_in[0] for db_data_2_in in db_data_2] if db_data_2 else []
            )
...
def admin_check(num=None, what=None, name=""):
    ...
    if ip_or_user(ip) == 0:
        curs.execute(
            db_change("select data from user_set where id = ? and name = 'acl'"), [ip]
        )
        ...
        if user_auth:
            ...
            curs.execute(
                db_change('select name from alist where name = ? and acl = "owner"'),
                [user_auth],
            )
            if curs.fetchall():
                pass_ok = 1
                ...
                if num == "all":
                    curs.execute(
                        db_change("select name from alist where name = ?"), [user_auth]
                    )
                else:
                    curs.execute(
                        db_change("select name from alist where name = ? and acl = ?"),
                        [user_auth, check],
                    )
    ...
    for i in range(0, end):
        if tool == "":
            ...
            if i == 0:
                ...
                curs.execute(
                    db_change('select data from other where name = "document_move_acl"')
                )
            ...
            if i == 0:
                ...
                curs.execute(
                    db_change('select data from other where name = "document_edit_acl"')
                )
            ...
            if i == 0:
                curs.execute(
                    db_change("select acl from rd where code = ?"), [topic_num]
                )
            elif i == 1:
                curs.execute(
                ...
                curs.execute(
                    db_change('select data from other where name = "discussion"')
                )
            ...
            curs.execute(
                db_change("select data from other where name = 'many_upload_acl'")
            )
            ...
            if i == 0:
                ...
                curs.execute(
                    db_change('select data from other where name = "vote_acl"')
                )

            num = None
        elif tool == "slow_edit":
            curs.execute(
                db_change('select data from other where name = "slow_edit_acl"')
            )
            ...
            if i == 0:
                ...
                curs.execute(
                    db_change("select data from other where name = 'all_view_acl'")
                )
        ...
        if acl_data[0][0] != "normal":
            if (
                not acl_data[0][0] in ["ban", "ban_admin"]
                and not tool in except_ban_tool_list
            ):
                if get_ban == 1:
                    return 1
            ...
            if acl_data[0][0] in ["all", "ban"]:
                ...
                if ip_or_user(ip) != 1:
                    if admin_check(num) == 1:
                        ...
                        curs.execute(
                            db_change("select count(*) from history where ip = ?"), [ip]
                        )
                        count = curs.fetchall()
                ...
                curs.execute(
                    db_change("select ip from history where title = ? and ip = ?"),
                    [name, ip],
                )
                if curs.fetchall():
                    return 0
...
def get_edit_text_bottom_check_box():
    ...
    curs.execute(
        db_change('select data from other where name = "copyright_checkbox_text"')
    )
    sql_d = curs.fetchall()
    if sql_d and sql_d[0][0] != "":
        cccb_text = (
            ""
            + '<input type="checkbox" name="copyright_agreement" value="yes"> '
            + sql_d[0][0]
            + '<hr class="main_hr">'
            + ""
        )
...
def do_edit_send_check(data):
    ...
    curs.execute(
        db_change('select data from other where name = "edit_bottom_compulsion"')
    )
    db_data = curs.fetchall()
    if db_data and db_data[0][0] != "":
...
def do_title_length_check(name, check_type="document"):
    ...
    if check_type == "topic":
        curs.execute(
            db_change('select data from other where name = "title_topic_max_length"')
        )
        db_data = curs.fetchall()
        if db_data and db_data[0][0] != "":
        ...
        curs.execute(
            db_change('select data from other where name = "title_max_length"')
        )
        db_data = curs.fetchall()
        if db_data and db_data[0][0] != "":
            db_data = int(number_check(db_data[0][0]))
...
def do_add_thread(thread_code, thread_data, thread_top="", thread_id=""):
    ...
    curs.execute(
        ...
        [thread_id, thread_data, get_time(), ip_check(), thread_top, thread_code],
    )
...
def do_reload_recent_thread(topic_num, date, name=None, sub=None):
    ...
    if curs.fetchall():
        curs.execute(
            db_change("update rd set date = ? where code = ?"), [date, topic_num]
        )
    else:
        curs.execute(
            db_change(
...
def add_alarm(who, context):
    ...
    curs.execute(
        db_change("insert into alarm (name, data, date) values (?, ?, ?)"),
...
def add_user(user_name, user_pw, user_email="", user_encode=""):
    ...
    curs.execute(
        db_change("insert into user_set (id, name, data) values (?, 'pw', ?)"),
        [user_name, user_pw_hash],
    )
    curs.execute(
        db_change("insert into user_set (id, name, data) values (?, 'acl', ?)"),
        [user_name, user_auth],
    )
    curs.execute(
        db_change("insert into user_set (id, name, data) values (?, 'date', ?)"),
        [user_name, get_time()],
    )
    curs.execute(
        db_change("insert into user_set (id, name, data) values (?, 'encode', ?)"),
        [user_name, data_encode],
    )

    if user_email != "":
        curs.execute(
            db_change("insert into user_set (name, id, data) values ('email', ?, ?)"),
...
def ban_insert(name, end, why, login, blocker, type_d=None):
    ...
    if curs.fetchall():
        curs.execute(
            ...
            [name, "release", now_time, blocker, "", band],
        )
        curs.execute(
            db_change(
        ...
        curs.execute(
            ...
            [name, r_time, now_time, blocker, why, band, login],
        )
...
def history_plus(title, data, date, ip, send, leng, t_check="", mode=""):
    ...
    curs.execute(
        db_change('select data from other where name = "history_recording_off"')
    )
    db_data = curs.fetchall()
    if db_data and db_data[0][0] != "":
        return 0
    ...
    if mode != "add":
        ...
        if curs.fetchall()[0][0] >= 200:
            ...
            if rc_data:
                curs.execute(
                    db_change("delete from rc where id = ? and title = ? and type = ?"),
                    [rc_data[0][0], rc_data[0][1], mode],
                )

        curs.execute(
            db_change("insert into rc (id, title, date, type) values (?, ?, ?, ?)"),
            [id_data, title, date, mode],
        )
    ...
    curs.execute(
        ...
        [id_data, title, data, date, ip, send, leng, mode],
    )
    ...
    curs.execute(
        db_change(
            "select doc_name from data_set where doc_name = ? and set_name = 'last_edit'"
        ),
        [title],
    )
    db_data = curs.fetchall()
    if db_data:
        curs.execute(
            db_change(
                "update data_set set set_data = ?, doc_rev = ? where doc_name = ? and set_name = 'last_edit'"
            ),
            [date, data_set_exist, title],
        )
    else:
        curs.execute(
            db_change(
...
def re_error(data):
    ...
    if data == "/ban":
        ...
        if num == 1:
            ...
            data = (
                load_lang("id_char_error")
                + ' <a href="/name_filter">('
                + load_lang("id_filter_list")
                + ")</a>"
            )
        elif num == 9:
            ...
            data = (
                load_lang("file_extension_error")
                + ' <a href="/extension_filter">('
                + load_lang("extension_filter_list")
                + ")</a>"
            )
        elif num == 15:
            ...
            data = (
                load_lang("update_error")
                + ' <a href="https://github.com/opennamu/opennamu">(Github)</a>'
            )
        elif num == 35:
            ...
            curs.execute(
                db_change(
                    "select data from other where name = 'title_topic_max_length'"
                )
            )
            db_data = curs.fetchall()
            db_data = "" if not db_data else db_data[0][0]
            ...
            curs.execute(
                db_change("select data from other where name = 'password_min_length'")
            )
            db_data = curs.fetchall()
            if db_data and db_data[0][0] != "":
        ...
        if num == 5:
            ...
            tool = [
                ["change", load_lang("user_setting")],
                ["change/skin_set/main", load_lang("main_skin_set")],
            ]
            load_skin_set = ""
