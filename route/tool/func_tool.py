    ...
    if d_type == 0 and (flask.session and "id" in flask.session):
        ...
        ip = flask.request.environ.get(
            ...
            flask.request.environ.get(
                "HTTP_CF_CONNECTING_IP", flask.request.environ.get("REMOTE_ADDR", "::1")
            ),
        )
...
def get_main_skin_set(curs, flask_session, set_name, ip):
    if ip_or_user(ip) == 0:
        curs.execute(
            db_change("select data from user_set where name = ? and id = ?"),
            [set_name, ip],
        )
        ...
        set_data = (
            flask_session[set_name]
            if set_name in flask_session and flask_session[set_name] != ""
            else "default"
        )
