...
def user_watch_list(tool):
    with get_db_connect() as conn:
        ...
        if tool == "watch_list":
            curs.execute(
                db_change("select title from scan where type = '' and user = ?"), [ip]
            )

            title_name = load_lang("watchlist")
        else:
            curs.execute(
                db_change("select title from scan where type = 'star' and user = ?"),
                [ip],
            )
        ...
        for data_list in data:
            ...
            div += (
                ...
                + url_pas(data_list[0])
                + '">'
                + html.escape(data_list[0])
                + "</a> "
                + plus
                + '<a href="/'
                + ("star_doc" if tool == "star_doc" else "watch_list")
                + "/"
                + url_pas(data_list[0])
                + '">('
                + load_lang("delete")
