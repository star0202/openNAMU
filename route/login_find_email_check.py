...
def login_find_email_check(tool):
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST" or (
            ...
            if (
                "c_type" in flask.session
                and flask.session["c_type"] == "pass_find"
                and flask.session["c_key"] == input_key
            ):
                curs.execute(
                    db_change(
                        "update user_set set data = ? where name = 'pw' and id = ?"
                    ),
                    [pw_encode(flask.session["c_key"]), flask.session["c_id"]],
                )
                ...
                if curs.fetchall():
                    curs.execute(
                        db_change(
                            "update user_set set data = '' where name = '2fa' and id = ?"
                        ),
                        [user_id],
                    )
                ...
                curs.execute(
                    db_change('select data from other where name = "reset_user_text"')
                )
                sql_d = curs.fetchall()
                b_text = (
                    (sql_d[0][0] + '<hr class="main_hr">')
                ...
                or flask.session["c_key"] == "email_pass"
            ):
                ...
                if flask.session["c_type"] == "register":
                    ...
                    curs.execute(
                        db_change("select id from user_set where id = ?"),
                        [flask.session["c_id"]],
                    )
                    if curs.fetchall():
                        for i in re_set_list:
                    ...
                    curs.execute(
                        db_change(
                            "select id from user_set where id = ? and name = 'application'"
                        ),
                        [flask.session["c_id"]],
                    )
                    if curs.fetchall():
                    ...
                    curs.execute(
                        ...
                        [flask.session["c_id"], flask.session["c_email"]],
                    )
                    ...
                    curs.execute(
                        db_change(
                            'delete from user_set where name = "email" and id = ?'
                        ),
                        [ip],
                    )
                    curs.execute(
            ...
            curs.execute(
                db_change('select data from other where name = "check_key_text"')
            )
            sql_d = curs.fetchall()
            b_text = (
                (sql_d[0][0] + '<hr class="main_hr">')
