...
def login_register_submit_2():
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change('select data from other where name = "approval_question"')
        )
        ...
        if flask.request.method == "POST":
            ...
            curs.execute(
                db_change("insert into user_set (id, name, data) values (?, ?, ?)"),
                [flask.session["submit_id"], "application", json.dumps(user_app_data)],
            )
            conn.commit()
