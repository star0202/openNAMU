    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            if ip_or_user(ip) == 0:
                ...
                if curs.fetchall():
                    ...
                    curs.execute(
                        ...
                        [ip, "custom_css", get_data],
                    )

                curs.execute(
                    db_change("select id from user_set where id = ? and name = ?"),
                    [ip, "custom_css_" + skin_name],
                )
                if curs.fetchall():
                    curs.execute(
                        db_change(
                        ...
                        [get_data, ip, "custom_css_" + skin_name],
                    )
                else:
                    curs.execute(
                        db_change(
                        ...
                        [ip, "custom_css_" + skin_name, get_data],
                    )
            ...
            if ip_or_user(ip) == 0:
                ...
                data_skin = (
                    flask.session["head_" + skin_name]
                    if "head_" + skin_name in flask.session
                    else ""
                )
