...
def view_acl(name):
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            acl_data += [
                ["document_edit_acl", flask.request.form.get("document_edit_acl", "")]
            ]
            acl_data += [
                ["document_move_acl", flask.request.form.get("document_move_acl", "")]
            ]
            acl_data += [
                [
                    "document_delete_acl",
                    flask.request.form.get("document_delete_acl", ""),
                ]
            ]
            ...
            for i in acl_data:
                curs.execute(
                    db_change("select title from acl where title = ? and type = ?"),
                    [name, i[0]],
                )
                if curs.fetchall():
                    curs.execute(
                        db_change(
                            "update acl set data = ? where title = ? and type = ?"
                        ),
                        [i[1], name, i[0]],
                    )
                else:
            ...
            if db_data:
                curs.execute(
                    db_change(
                        "update data_set set set_data = ? where doc_name = ? and set_name = 'document_markup'"
                    ),
                    [markup_data, name],
                )
            else:
                curs.execute(
                    db_change(
                        "insert into data_set (doc_name, doc_rev, set_name, set_data) values (?, '', 'document_markup', ?)"
                    ),
                    [name, markup_data],
                )
            ...
            if not db_data or db_data[0][0] != markup_data:
                ...
                if db_data_2:
                    render_set(
                        doc_name=name, doc_data=db_data_2[0][0], data_type="backlink"
                    )
            ...
            if user_page == 1:
                admin_check(
                    5, check_data + " (" + all_d + ")" + " (" + markup_data + ")"
                )
            ...
            acl_list = (
                get_acl_list("user") if re.search(r"^user:", name) else get_acl_list()
            )
            if not re.search(r"^user:", name):
                ...
                acl_get_list = [[load_lang("document_acl"), "decu", "2"]]
            ...
            for i in acl_get_list:
                ...
                curs.execute(
                    db_change("select data from acl where title = ? and type = ?"),
                    [name, i[1]],
                )
                ...
                for data_list in acl_list:
                    ...
                    data += (
                        ...
                        + check
                        + ">"
                        + (data_list if data_list != "" else "normal")
                        + "</option>"
                    )
            ...
            data += (
                ...
                + """</li>
                    <li>before : """
                + load_lang("before_acl")
                + """</li>
                    <li>30_day : """
                + load_lang("30_day_acl")
                + """</li>
                    <li>ban_admin : """
                + load_lang("ban_admin_acl")
                + """</li>
                    <li>not_all : """
                + load_lang("not_all_acl")
                + """</li>
                    <li>90_day : """
                + load_lang("90_day_acl")
                + """</li>
                </ul>
                <hr class="main_hr">
                <h2>"""
