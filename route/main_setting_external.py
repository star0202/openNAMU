    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            for i in ["tls", "starttls", "plain"]:
                if d_list[4] == i:
                    security_radios = (
                        '<option value="' + i + '">' + i + "</option>" + security_radios
                    )
                else:
                    security_radios += '<option value="' + i + '">' + i + "</option>"

            re_ver_list = {"": "reCAPTCHA v2", "v3": "reCAPTCHA v3", "h": "hCAPTCHA"}
            re_ver = ""
            for i in re_ver_list:
                if d_list[7] == i:
                    ...
                    re_ver += (
                        '<option value="' + i + '">' + re_ver_list[i] + "</option>"
                    )
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    <form method="post">
                        ...
                        + load_lang("recaptcha")
                        + """)</a> <a href="https://www.hcaptcha.com/">("""
                        + load_lang("hcaptcha")
                        + """)</a>
                        <hr class="main_hr">
                        ...
                        + """</h2>
                        <a href="/setting/phrase#s-6">("""
                        + load_lang("text_setting")
                        + """)</a>
                        <hr class="main_hr">

                        <input type="checkbox" name="email_have" """
                        + ("checked" if d_list[9] != "" else "")
                        + """> """
                        ...
                        + '''</span>
                        <hr class="main_hr">
                        <input name="oauth_client_id" value="'''
