    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == "POST":
            ...
            if data:
                ...
                history_plus(
                    name, "", today, ip, send, leng, t_check="delete", mode="delete"
                )
                ...
                for data in curs.fetchall():
                    curs.execute(
                        db_change(
                            "insert into back (title, link, type) values (?, ?, 'no')"
                        ),
                        [data[0], data[1]],
                    )
            ...
            curs.execute(
                db_change("update other set data = ? where name = 'count_all_title'"),
