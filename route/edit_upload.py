...
def edit_upload():
    with get_db_connect() as conn:
        ...
        file_max = (
            number_check(db_data[0][0]) if db_data and db_data[0][0] != "" else "2"
        )
        ...
        if flask.request.method == "POST":
            ...
            for data in file_data:
                ...
                curs.execute(
                    db_change("select html from html_filter where kind = 'extension'")
                )
                ...
                if flask.request.form.get("f_name", None):
                    name = (
                        flask.request.form.get("f_name", None)
                        + (" " + str(file_num) if file_num else "")
                        + value
                    )
                else:
                    name = data.filename
                ...
                curs.execute(
                    db_change("select html from html_filter where kind = 'file'")
                )
                db_data = curs.fetchall()
                for i in db_data:
                    t_re = re.compile(i[0])
                ...
                if db_data and db_data[0][0] == "namumark":
                    file_d = (
                        ...
                        + "\n"
                        + "[[category:"
                        + re.sub(r"\]", "_", flask.request.form.get("f_lice_sel", ""))
                        + "]]\n"
                ...
                render_set(
                    doc_name="file:" + name, doc_data=file_d, data_type="backlink"
                )
            ...
            license_list = (
                ...
                + load_lang("direct_input")
                + "</option>"
            )
            file_name = html.escape(flask.request.args.get("name", ""))

            curs.execute(
                db_change("select html from html_filter where kind = 'image_license'")
            )
            db_data = curs.fetchall()
            license_list += "".join(
                ['<option value="' + i[0] + '">' + i[0] + "</option>" for i in db_data]
            )

            curs.execute(db_change("select data from other where name = 'upload_help'"))
            db_data = curs.fetchall()
            upload_help = (
                ('<hr class="main_hr">' + db_data[0][0])
                if db_data and db_data[0][0] != ""
                else ""
            )

            curs.execute(
                db_change("select data from other where name = 'upload_default'")
            )
            db_data = curs.fetchall()
            upload_default = (
                html.escape(db_data[0][0]) if db_data and db_data[0][0] != "" else ""
            )
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    data="""
                    <a href="/file_filter">("""
                    + load_lang("file_filter_list")
                    + """)</a> <a href="/extension_filter">("""
                    ...
                    + '''
                        ...
                        <hr class="main_hr">
                        <textarea class="opennamu_textarea_100" placeholder="'''
                    + load_lang("other")
                    + """" name="f_lice">"""
                    + upload_default
                    + """</textarea>
