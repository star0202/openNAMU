...
def vote_select(num=1):
    ...
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change("select user from vote where id = ? and user = ?"),
            [num, ip_check()],
        )
        if curs.fetchall():
            return redirect("/vote/end/" + num)
        ...
        if flask.request.method == "POST":
            ...
            data += (
                "<b>" + data_list[0][1] + '</b><hr class="main_hr">'
                if data_list[0][1] != ""
                else ""
            )
            data += (
                "<span>~ " + time_limit + '</span><hr class="main_hr">'
                if time_limit != ""
                else ""
            )
            ...
            for i in vote_data:
                select_data += (
                    '<option value="' + str(line_num) + '">' + i + "</option>"
                )
                line_num += 1
            ...
            return easy_minify(
                flask.render_template(
                    ...
                    data=data,
                    menu=[
                        ["vote", load_lang("return")],
                        ["vote/end/" + num, load_lang("result")],
                    ],
                )
            )
