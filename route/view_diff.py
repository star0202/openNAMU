...
def view_diff(name="Test", num_a=1, num_b=1):
    with get_db_connect() as conn:
        ...
        curs.execute(
            db_change(
                "select title from history where title = ? and (id = ? or id = ?) and hide = 'O'"
            ),
            [name, first, second],
        )
        ...
        curs.execute(
            db_change("select data from history where id = ? and title = ?"),
            [first, name],
        )
        ...
        if first_raw_data and second_raw_data:
            ...
            if first_raw_data == second_raw_data:
                ...
                for re_in_data in re_data:
                    ...
                    if (
                        change_count != 0
                        or change_find_start != 0
                        or change_find_end != 0
                    ):
