...
if what_i_do == "1":
    ...
    curs.execute(
        db_change("update user_set set id = ? where id = ?"), [new_name, user_name]
    )
elif what_i_do == "11":
    ...
    curs.execute(
        db_change("update user_set set data = 'owner' where id = ? and name = 'acl'"),
        [user_name],
    )
elif what_i_do == "16":
    print("----")
    ...
    curs.execute(
        db_change('select data from user_set where name = "2fa" and id = ?'),
        [user_name],
    )
    if curs.fetchall():
        curs.execute(
            db_change("update user_set set data = '' where name = '2fa' and id = ?"),
            [user_name],
        )
elif what_i_do == "17":
    print("----")
    ...
    curs.execute(
        db_change("update other set data = ? where name = 'wiki_access_password'"),
    ...
    if platform.system() == "Linux":
        ...
        ok += [
            os.system("git remote add origin https://github.com/opennamu/opennamu.git")
        ]
        ...
        urllib.request.urlretrieve(
            "https://github.com/opennamu/opennamu/archive/" + up_data + ".zip",
            "update.zip",
        )
    ...
    curs.execute(
        db_change('insert into other (name, data, coverage) values ("domain", ?, "")'),
        [domain],
    )
elif what_i_do == "21":
    print("----")
    ...
    curs.execute(
        db_change(
            'insert into other (name, data, coverage) values ("http_select", ?, "")'
        ),
        [tls_v],
    )
else:
