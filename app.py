...
with get_db_connect() as conn:
            ...
            for create in ["test"] + create_data[create_table]:
                ...
                if db_pass == 0:
                    try:
                        curs.execute(
                            db_change(
                                "create table " + create_table + "(test longtext)"
                            )
                        )
    ...
    for i in server_set_var:
        ...
        if server_set_val:
            ...
            if "list" in server_set_var[i]:
                print(
                    server_set_var[i]["display"]
                    + " ("
                    + server_set_var[i]["default"]
                    + ") ["
                    + ", ".join(server_set_var[i]["list"])
                    + "]"
                    + " : ",
                    end="",
                )
    ...
    if data_db_set["type"] == "sqlite":
        ...
        def back_up(back_time, back_up_where):
            ...
            try:
                shutil.copyfile(data_db_set["name"] + ".db", back_up_where)
            ...
            threading.Timer(
                60 * 60 * back_time, back_up, [back_time, back_up_where]
            ).start()

        curs.execute(db_change('select data from other where name = "back_up"'))
        back_time = curs.fetchall()
        back_time = (
            int(number_check(back_time[0][0])) if back_time and back_time != "" else 0
        )
        if back_time != 0:
            curs.execute(
                db_change('select data from other where name = "backup_where"')
            )
...
app.route("/inter_wiki/del/<everything:name>", defaults={"tool": "del_inter_wiki"})(
    filter_inter_wiki_delete
)
app.route(
    "/inter_wiki/add", methods=["POST", "GET"], defaults={"tool": "plus_inter_wiki"}
)(filter_inter_wiki_add)
app.route(
    "/inter_wiki/add/<everything:name>",
    methods=["POST", "GET"],
    defaults={"tool": "plus_inter_wiki"},
)(filter_inter_wiki_add)

app.route("/filter/document/list")(filter_document)
app.route("/filter/document/add/<everything:name>", methods=["POST", "GET"])(
    filter_document_add
)
...
app.route("/edit_top/del/<everything:name>", defaults={"tool": "del_edit_top"})(
    filter_inter_wiki_delete
)
app.route("/edit_top/add", methods=["POST", "GET"], defaults={"tool": "plus_edit_top"})(
    filter_inter_wiki_add
)

app.route("/image_license", defaults={"tool": "image_license"})(filter_inter_wiki)
app.route(
    "/image_license/del/<everything:name>", defaults={"tool": "del_image_license"}
)(filter_inter_wiki_delete)
app.route(
    "/image_license/add",
    methods=["POST", "GET"],
    defaults={"tool": "plus_image_license"},
)(filter_inter_wiki_add)

app.route("/edit_filter", defaults={"tool": "edit_filter"})(filter_inter_wiki)
app.route("/edit_filter/del/<everything:name>", defaults={"tool": "del_edit_filter"})(
    filter_inter_wiki_delete
)
app.route(
    "/edit_filter/add", methods=["POST", "GET"], defaults={"tool": "plus_edit_filter"}
)(filter_inter_wiki_add)
app.route(
    "/edit_filter/add/<everything:name>",
    methods=["POST", "GET"],
    defaults={"tool": "plus_edit_filter"},
)(filter_inter_wiki_add)

app.route("/email_filter", defaults={"tool": "email_filter"})(filter_inter_wiki)
app.route("/email_filter/del/<everything:name>", defaults={"tool": "del_email_filter"})(
    filter_inter_wiki_delete
)
app.route(
    "/email_filter/add", methods=["POST", "GET"], defaults={"tool": "plus_email_filter"}
)(filter_inter_wiki_add)

app.route("/file_filter", defaults={"tool": "file_filter"})(filter_inter_wiki)
app.route("/file_filter/del/<everything:name>", defaults={"tool": "del_file_filter"})(
    filter_inter_wiki_delete
)
app.route(
    "/file_filter/add", methods=["POST", "GET"], defaults={"tool": "plus_file_filter"}
)(filter_inter_wiki_add)

app.route("/name_filter", defaults={"tool": "name_filter"})(filter_inter_wiki)
app.route("/name_filter/del/<everything:name>", defaults={"tool": "del_name_filter"})(
    filter_inter_wiki_delete
)
app.route(
    "/name_filter/add", methods=["POST", "GET"], defaults={"tool": "plus_name_filter"}
)(filter_inter_wiki_add)

app.route("/extension_filter", defaults={"tool": "extension_filter"})(filter_inter_wiki)
app.route(
    "/extension_filter/del/<everything:name>", defaults={"tool": "del_extension_filter"}
)(filter_inter_wiki_delete)
app.route(
    "/extension_filter/add",
    methods=["POST", "GET"],
    defaults={"tool": "plus_extension_filter"},
)(filter_inter_wiki_add)
...
app.route("/list/admin/auth_use/<arg_search>/<int:arg_num>", methods=["POST", "GET"])(
    list_admin_auth_use
)
...
app.route(
    "/auth/give/ban_regex/<everything:name>",
    methods=["POST", "GET"],
    defaults={"ban_type": "regex"},
)(give_user_ban)
app.route(
    "/auth/give/ban_multiple",
    methods=["POST", "GET"],
    defaults={"ban_type": "multiple"},
)(give_user_ban)
...
app.route(
    "/history/<everything:name>", defaults={"tool": "history"}, methods=["POST", "GET"]
)(recent_change)
app.route("/history_tool/<int(signed = True):rev>/<everything:name>")(
    recent_history_tool
)
app.route(
    "/history_delete/<int(signed = True):rev>/<everything:name>",
    methods=["POST", "GET"],
)(recent_history_delete)
app.route("/history_hidden/<int(signed = True):rev>/<everything:name>")(
    recent_history_hidden
)
app.route(
    "/history_send/<int(signed = True):rev>/<everything:name>", methods=["POST", "GET"]
)(recent_history_send)
app.route("/history_reset/<everything:name>", methods=["POST", "GET"])(
    recent_history_reset
)
app.route("/history_add/<everything:name>", methods=["POST", "GET"])(recent_history_add)
app.route(
    "/history_add_preview/<everything:name>",
    defaults={"do_type": "preview"},
    methods=["POST"],
)(recent_history_add)
...
app.route(
    "/diff/<int(signed = True):num_a>/<int(signed = True):num_b>/<everything:name>"
)(view_diff)
...
app.route(
    "/edit_preview/<everything:name>", defaults={"do_type": "preview"}, methods=["POST"]
)(edit)
app.route("/edit_from/<everything:name>", defaults={"do_type": "load"})(edit)
app.route("/edit_section/<int:section>/<everything:name>", methods=["POST", "GET"])(
    edit
)
app.route(
    "/edit_section_preview/<int:section>/<everything:name>",
    defaults={"do_type": "preview"},
    methods=["POST"],
)(edit)
...
app.route(
    "/thread/0/<everything:doc_name>",
    defaults={"topic_num": "0"},
    methods=["POST", "GET"],
)(topic)
app.route(
    "/thread_preview/<int:topic_num>", defaults={"do_type": "preview"}, methods=["POST"]
)(topic)
...
app.route("/thread/<int:topic_num>/comment/<int:num>/delete", methods=["POST", "GET"])(
...
app.route("/change/head", methods=["GET", "POST"], defaults={"skin_name": ""})(
    user_setting_head
)
...
app.route("/watch_list/<everything:name>", defaults={"tool": "watch_list"})(
    user_watch_list_name
)

app.route("/star_doc", defaults={"tool": "star_doc"})(user_watch_list)
app.route("/star_doc/<everything:name>", defaults={"tool": "star_doc"})(
    user_watch_list_name
)
...
app.route("/login/find/email", methods=["POST", "GET"], defaults={"tool": "pass_find"})(
    login_find_email
)
app.route(
    "/login/find/email/check", methods=["POST", "GET"], defaults={"tool": "check_key"}
...
app.route(
    "/bbs/edit/preview/<int:bbs_num>/<int:post_num>",
    methods=["POST", "GET"],
    defaults={"do_type": "preview"},
)(bbs_edit)
app.route(
    "/bbs/w/preview/<int:bbs_num>/<int:post_num>",
    methods=["POST"],
    defaults={"do_type": "preview"},
)(bbs_w_post)
...
# Func-api
app.route("/api/w/<everything:name>/doc_tool/<tool>/doc_rev/<int(signed = True):rev>")(
    api_w
)
...
app.route("/manager/<int:num>/<everything:add_2>", methods=["POST", "GET"])(
    main_tool_redirect
)
...
app.route("/search/<int:num>/<everything:name>", methods=["POST", "GET"])(
    main_search_deep
)
app.route(
    "/search_data/<everything:name>",
    defaults={"search_type": "data"},
    methods=["POST", "GET"],
)(main_search_deep)
app.route(
    "/search_data/<int:num>/<everything:name>",
    defaults={"search_type": "data"},
    methods=["POST", "GET"],
)(main_search_deep)
...
app.route(
    "/setting/main", defaults={"db_set": data_db_set["type"]}, methods=["POST", "GET"]
)(main_setting_main)
...
app.route("/setting/head", defaults={"num": 3}, methods=["POST", "GET"])(
    main_setting_head
)
app.route("/setting/head/<skin_name>", defaults={"num": 3}, methods=["POST", "GET"])(
    main_setting_head
)
app.route("/setting/body/top", defaults={"num": 4}, methods=["POST", "GET"])(
    main_setting_head
)
app.route(
    "/setting_preview/body/top", defaults={"num": 4, "set_preview": 1}, methods=["POST"]
)(main_setting_head)
app.route("/setting/body/bottom", defaults={"num": 7}, methods=["POST", "GET"])(
    main_setting_head
)
app.route(
    "/setting_preview/body/bottom",
    defaults={"num": 7, "set_preview": 1},
    methods=["POST"],
)(main_setting_head)
